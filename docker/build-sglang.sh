...
install_toolchain() {
    apt-get update
    apt-get install -y --no-install-recommends git curl ca-certificates build-essential python3-pip
...
enable_gpu_free_setup() {
    ...
    grep -qF "assert torch.cuda.is_available(), 'CUDA not available';" "$setup"
    sed -i "s|assert torch.cuda.is_available(), 'CUDA not available'; ||" "$setup"
...
apply_tp1_store_cache_fallback() {
    ...
    grep -q '^def can_use_store_cache(size: int) -> bool:$' "$kvcache"
    ...
    grep -A2 '^def can_use_store_cache(size: int) -> bool:$' "$kvcache" | grep -q 'SGLANG_RDNA4_DISABLE_STORE_CACHE'
    grep -A3 '^def can_use_store_cache(size: int) -> bool:$' "$kvcache" | grep -q 'return False  # RDNA4 TP=1'
}
...
case "${1:-}" in
    ...
    build-sglang) build_sglang ;;
    *) echo "Usage: $0 {install-toolchain|build-sglang}" >&2; exit 64 ;;
esac
