...
**TRITON-CACHE BISECT (2026-05-26): kernel exonerated, found dropped patch hunk + 2nd squash.** Per-version caches tri-v11/tri-v12: gptq_awq compiled meta identical (warps4/stages2/shared6144), amdgcn ~match. v0.5.12 `get_default_config` int4 small-M branch (M≤E = single-user decode) lost `BLOCK_SIZE_N` (RDNA4 patch dropped in rebase) → fresh-cache `dynamic_func missing BLOCK_SIZE_N` crash / stale-cache wrong tile. Restored `BLOCK_SIZE_N:32` → crash gone, BUT reap still `****` squash → 2nd bug is weight/scale prep (awq.py/layer.py), not kernel. Re-add hunk to patch 004. SQUASH ISOLATED: not kernel/config/fused_moe/_load_w13 (all identical v11↔v12.orig) → patch **004-rdna4-moe-fixes FULLY REJECTS on v0.5.12** (verified: clean re-clone aborts, 4 .rej incl fused_moe_triton_config BLOCK_SIZE_N hunk + layer.py moved-import). All 004 MoE fixes drop → crash + squash. 003/011 also fail. Setup's "8 hunks, 2 rebased" was wrong — 004 never applied. FIX: rebase 003/004/011 to v0.5.12. **A/B CONFIRMED (2026-05-26): SGLang regression, NOT torch.** Built env `sglang-0511-bisect` = v0.5.11 + commit `96160e0` patches on torch 2.11 (same as broken v0.5.12 env). reap-25b coder MoE → clean correct `fib()`, finish=stop. Same native-AWQ is gibberish on v0.5.12. Both torch 2.11 ⇒ torch exonerated. And `fused_moe_kernel_gptq_awq` + `moe_wna16.py` are byte-identical v11→v12 (only SPDX + gelu assert) ⇒ regression is in another MoE file (layer.py / triton_kernels_moe / ep_moe / awq scheme / moe_runner). Bisect those next. (Tried matching the 2.12 nightly: 0310 GC'd, 0328 segfaults on import — dead line; 2.11 is the shared control.)

The earlier verdict was "v0.5.11 MoE coherent, v0.5.12 gibberish (20× expert squash) — kernel regression in int4 fused experts." To diff working-vs-broken kernels directly, building a parallel known-good env from repo commit `96160e0` (which pinned `SGLANG_TAG=v0.5.11`, same triton 3.6 + 2.11 torch, only sglang version differs): worktree `/data/sglang-0511-bisect`, conda env `sglang-0511-bisect`, build log `/tmp/sgl0511-build-logs/run.log`. Plan: run an int4-MoE coder on v0.5.11 (expect coherent) and v0.5.12 (gibberish), then diff `fused_moe_triton_kernels.py:fused_moe_kernel_gptq_awq` + the dequant path between tags to pinpoint the regression hunk. Earlier "0.5.10" was a misremember; 96160e0 is 0.5.11, which is the right working anchor anyway.
...
- **Two launch presets broken on TP1 small-ctx probes (2026-05-26).** `gemma4` hardcoded `--tokenizer-path .../gemma-4-26B-A4B-it-BF16` (reference dir, no longer on disk) → crashloop; fixed to fall back to the AWQ dir's bundled tokenizer. `qwen36-moe` ignores `CTX=2048 TP=1` overrides and crashloops at 256K/0.85-mem on one card — use TP2 or `--context-length` explicitly. **MAJOR (2026-05-26): gemma-4-26B (128exp) gibberish TOO** — `'-f(n):-n(n)'`, 0 unbound, same repeat as coder → **shared moe_wna16 AWQ regression, not coder-specific** (v0.5.12 touched moe_wna16/fused_moe). 35B 3090-rebuilt (skip). **SILU EXONERATED:** torch fallback `silu(x[:d])*x[d:]` → gemma SAME gibberish, so 20× squash is GEMM1/dequant (w13), not activation. Bisect: AWQ dequant vs v0.5.11; EXPERT_DUMP gemm1/silu absmax.
- **256K sweep 2026-05-26: DENSE 4/4 PASS, MoE all gibberish (`fused_moe_kernel_gptq_awq`).** TP2/262144: Devstral-24B basic+vision+tool ✅, gemma-4-31B basic+thinking+tool ✅ (vision degraded=calib), Qwen3.5-27B basic+thinking ✅, Qwen3-VL-32B basic+thinking+vision 3/3 ✅. Dense AWQ dequant is fine. **Every int4 MoE (coder/gemma-26B) → 20× expert-out squash → `:`/`(n):` repeat.** Dense-AWQ works ⇒ bug is the fused per-expert int4 path `fused_moe_triton_kernels.py:fused_moe_kernel_gptq_awq` (RDNA4 patch only adds fp8 kernel; int4 runs stock v0.5.12). v0.5.11 was coherent → v0.5.12 kernel regression. Fix: restore pre-bump kernel or per-expert unfused awq. Validator needs `pillow`. eval env=`sglang-triton36`.
