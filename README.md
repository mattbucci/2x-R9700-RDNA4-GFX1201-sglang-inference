...
2. **REAM/REAP coverage gaps (3090 audit, 2026-05-02).** Coverage matrix below. Open builds: clean text-only REAP for Qwen3.6-35B-A3B (#60), REAM or REAP for Gemma 4 26B (#61), REAM merger fix for Qwen3MoeForCausalLM to unblock Coder-30B-REAM (#62). 3090 has Cerebras Qwen3.5-28B-A3B-REAP recal IN FLIGHT (ETA ~18:00 PDT 2026-05-02) → R9700 cross-validates on publish (#59).

   | MoE base | Original AWQ | REAM | REAP |
   |----------|:------------:|:----:|:----:|
   | Qwen3.6-35B-A3B (256 exp) | ✅ | ✅ `Qwen3.6-REAM-A3B-AWQ` | ⚠️ only `VL-REAP-26B-A3B-AWQ` (vision tower stripped) — task #60 |
   ...
   | Qwen3.5-35B-A3B | ❌ unshipped | ❌ no shipper | ⏳ 3090 recal in flight — task #59 |
...
4. **Calibration-quality audit across all self-calibrated models.** The fixed shared_expert substring bug + the new llava_instruct silent fallback suggest other recipes may carry similar pitfalls. Audit: print effective `ignore=[...]` vs saved `model.safetensors` for Devstral, Gemma4-26B/31B, Coder-30B, Qwen3.5-27B/35B — flag anything that should have stayed BF16 but got INT4. One-shot diagnostic: `python3 -c "import json; print(json.load(open(f'{m}/config.json'))['quantization_config']['ignore'])" `.
