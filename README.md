...
- **Vision support** — Devstral-24B, Qwen3.5-27B, Gemma 4 31B: vision WORKING. Gemma 4 26B MoE: re-calibrated AWQ + upstream multimodal pipeline ported (gemma4_mm.py, gemma4_vision.py, gemma4_audio.py, clippable_linear.py). Text works without `--enable-multimodal`. With multimodal: model loads (9 GB/GPU) but HSA crash during vision encoder prefill — likely Triton bidirectional attention in vision encoder on RDNA4. Same crash pattern as other SWA/vision attention on RDNA4.
