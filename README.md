...
In priority order:
0. **MoE-coder gibberish — YaRN EXONERATED (2026-05-25): broken at 4K ctx, not just 262K.** Live REAM server @4096 ctx returns gibberish (`'n:n:nn:nn'`, `'isisisis'`) — rope scaling doesn't engage until ~32K, so YaRN/262K is NOT the cause. moe_wna16 also exonerated: 8-expert grouped fused_experts_impl ratio 1.0000. 96-exp grouped exact (ratio 0.99995). **RC LOCALIZED (5-25): MoE delta=0.0000 — experts() passthrough.** Live MOE_DBG: routing healthy (ids vary, rmax~7.5) but `final_hidden==input` exactly → experts add nothing, only attn survives → prompt repeat. No NaN, fused qk_norm_rope exonerated. Kernel proven nonzero in-proc → live binding gives 0. Weights bound (w13_qweight 255, scales .06, zeros 136), twmax .14, gs128, backend=TRITON. Ruled out: hybrid (skinny ext unimportable→always TRITON fused_func), num_experts=96, filter_expert. 3-stage cut: GEMM1 ic1max~31 ✅, down ic3max~11 ✅, final delta=0.0000 → MoE contribution cancels to constant → token repeat. Fixed dormant silu/gelu fallback arg-swap (only bites no-sgl_kernel boxes). RC open: experts net-zero on tiny acts; suspect w13/w2 scale orientation or repack at full-model scale (verifier exact in isolation). **A/B sharpened (2026-05-26): native `Coder-30B-AWQ` (128 exp, CT-labeled) serves CLEAN code; only the 128→96-pruned REAM/REAP go gibberish → trigger is the PRUNE, not coder weights.** **KERNEL EXONERATED (2026-05-26): live-dumped expert tensors → MoE out === input bit-for-bit (cos 1.0). Offline replay through fused_experts_impl gives out std 1e-5 absmax 2e-4 on real acts, but scales LINEARLY to 3.2 at input×100 → kernel is correct, expert output is just ~1000× too small to matter vs h.std=0.009 → residual passthrough → `(n):(n):` repeat.** Weights bound (qweight 255, scale .06, zeros 136), s1 mean .0079, gs128, 8/96 top-k healthy (twmax .17). RC = expert-output too small at the **96-expert prune dim**: scales are NOT the defect (REAM expert0 down/gate/up = .054/.024/.019 absmax IDENTICAL to native-128) — so build is fine; 128-exp serves clean, 96-exp underdelivers. moe_wna16 has no tuned config for E=96,N=192 (logs "default, sub-optimal"). Tool: `scripts/debug/moe_live_dump_replay.py`. **Pad-to-128 test (2026-05-26): ne=96 vs ne=128 both out ≈0 → NOT expert count.** New lead: live MoE INPUT h.std=0.009 — post-RMSNorm should be ~1; experts faithfully pass a tiny input, so MoE is exonerated and the collapse is UPSTREAM (input_layernorm/embed scale or attn output). Next: trace per-layer hidden absmax from embed→layer0 on REAM vs native-128; find where magnitude drops to ~.009; gemma re-probe skip_special_tokens=false; regen patch 001/003/004/011 for v0.5.12. Then: regen patch 001/003/004/011 for v0.5.12; gemma re-probe skip_special_tokens=false.
