...
1. **Expand spec-decode coverage — land a working draft on as many models as we can.** Status tracked in [`benchmarks/specdecode.json`](benchmarks/specdecode.json) → fleet chart [`benchmarks/specdecode_fleet.png`](benchmarks/specdecode_fleet.png). **7 already working** — the whole A3B-MoE coder/REAM/REAP family rides EAGLE3/DFlash transfer drafts at 256K (Qwen3.6-REAM-A3B 133, Coder-30B-REAM 101, Coder-30B 97, REAP-25B 91, Coder-30B-REAP 90, Qwen3.6-35B-A3B 80; Coder-Next-REAM 58 @64K). Remaining targets: **(untested — a draft path exists, just bench)** Qwen3-Coder-Next-80B (Aurora-Spec EAGLE3); **(blocked — needs work)** Qwen3.5-28B-A3B-REAP — **tested 2026-06-14, no useful speedup** (downloaded `z-lab/Qwen3.5-35B-A3B-DFlash` parent draft, served via the qwen35-moe preset with `--speculative-attention-mode decode`: loads, coherent, 4/4 caps incl. vision+video, accepts tokens — accept len 2–8 workload-dependent). **Authoritative server-log sustained gen-throughput ≈52 tok/s at short ctx vs the ~60 no-spec baseline → net-negative-to-neutral**: the DFlash block-diffusion draft+verify overhead roughly cancels the acceptance benefit, and decode still collapses at deep context (≤~2 tok/s by ~240K — same DeltaNet+DFlash deep-context wall as Coder-Next-REAM; at 256K with chunked-prefill 8192 a verify child OOM-SIGQUITs, cleared at cp2048 but still no win). Corroborates the 3090's "DFlash moot on the Qwen DeltaNet-MoE family." Not a viable spec win — reuse not recommended. ⚠ **Measurement note:** the streaming-client TPOT path (`measure_decode_curve.py`) **under-measures bursty spec decode ~2×** (SSE buffering of accepted blocks) — it read 25.5@128 here where the server-log gen-throughput is ~52; **read the server-log `gen throughput` for spec, not client TPOT.** DeltaNet-dense 27B pair (Qwen3.5/3.6-27B: **EAGLE3 tested 2026-06-14 — loads+coherent+accepts (len 4.0) after patch 055, but net-negative ~18 vs ~25 no-spec; root cause = DeltaNet recurrent verify is sequential in num-draft-tokens so the spec step is compute-bound, cuda-graph no help, no tree-tuning fixes it; extends "DFlash moot on DeltaNet" to EAGLE3**. DFlash also OOMs @16K. Only lever = a DeltaNet-aware parallel verify kernel), gemma-4-31B/26B (no DFLASH hook; FROZEN_KV_MTP needs transformers 5.8 + an SGLang fix; base EAGLE3 accept only 1.34 → needs an it-matched draft), Qwen3-VL-32B (**blocker shifted 2026-06-14**: the [#17935](https://github.com/sgl-project/sglang/issues/17935) `get_embed_and_head` gap is gone — the method is present + patch 055 fixes the VL EAGLE3 capture marking, so the path now loads; real remaining gap = **no off-the-shelf draft** — the only Qwen3-VL eagle3 on HF is 2B, geometry-mismatched, and RedHatAI's 32B eagle3 targets the *text* model. **Highest-value spec target left:** VL-32B is pure dense (no DeltaNet verify wall) so EAGLE3 would actually *win* (~2× like Coder-30B), unlike the 27B — gated only on a SpecForge-trained 32B-matched draft) + VL-REAP-26B (SGLang VL spec bug #17935), Devstral-24B (no published draft → SpecForge-train a `MistralForCausalLMEagle`), Nemotron-Omni (no bundled draft, Nano excluded from MTP). Also **re-measure the existing AWQ + FP8 spec bars under cuda-graph** (current spec numbers predate the 2026-06-01 graph-ON sweep). **Spot-checked 2026-06-14 (cuda-graph ON, authoritative server-log gen-throughput):** both headline working drafts re-confirmed **healthy on the current stack** — Coder-30B AWQ + EAGLE3 = **~128 tok/s short-ctx, accept 6.7** (vs 56 no-spec = 2.3×; consistent with the documented ~97 @256K), and Qwen3.6-35B AWQ + DFlash = **~60 tok/s short-ctx, accept ~2.6** (≈ the 60 no-spec short — DFlash is neutral-by-design at short ctx; its 80 @256K win is long-ctx KV-amortization, where no-spec collapses to ~20). So no client-under-measurement error in the EAGLE3 bar; a full **256K** per-bar re-sweep (to re-confirm the @256K figures themselves) remains lower-priority polish. As each lands, flip its `specdecode.json` status to `working` + regenerate both charts (`scripts/bench/generate_charts.py`).
...
| Devstral-Small-2-24B-FP8 | tool path fixed — full run pending | **Tool-call path validated live 2026-06-15** (patches 040+056): a 2-turn probe (`read` → `todowrite` multi-token, *after a prior tool turn*) returns structured `tool_calls` with **no `[TOOL_CALLS]`/`[ARGS]` leak** into content (`scripts/eval/devstral2_toolprobe.py`; receipt `benchmarks/swebench/devstral2-fp8-toolprobe-2026-06-15.txt`). The earlier 0/15 was the `[TOOL_CALLS]`-omission + multi-token-name (`todowrite`/`webfetch`) gaps — closed by patch 040 (anchor on `[ARGS]`) + 056 (prefix-match the streamed name); the clean multi-turn case no longer reproduces the tokenization failure. Full buildable-15 SWE-bench re-run pending to un-defer (68% SWE-bench Verified base — would likely lead). |
