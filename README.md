...
| Qwen3.6-27B | DeltaNet+attn hybrid (VL) | 262K / 256K | 25 / 12.8 | `launch.sh qwen36-27b` | Working (native AWQ); 64 layers in 3:1 linear/full pattern. **AWQ 25 @128 / 13.9 @131K** post the 2026-05-31 GEMV batched-load fix (patch 006; was 14.5/9.9 — see Open work #2; other dense/hybrid AWQ rows pending re-sweep). FP8 same path as Qwen3.5-27B (shared `qwen3_5` file): native 20.82 GB/card, basic+thinking+VISION 3/3 PASS, 256K via patch 045 + cp2048, 9.3 tok/s @256K. No working draft (same block as Qwen3.5-27B). |
