...
In priority order:
0. **MoE-coder gibberish — RC pinned to per-expert AWQ dequant/bind, both backends.** Kernel (`fused_moe_kernel_gptq_awq`) is upstream-clean; topk + HIP GEMV exonerated; scales clean (2/14016). Checkpoints are **unfused** per-expert (`gate_proj`/`up_proj`/`down_proj`); SGLang fuses gate+up→w13 for moe_wna16. Next-step plan: (a) build expert0 dequant verifier — load coder w13/w2 the way moe_wna16 binds them, dequant via python ref, compare absmax/dist to bf16 base; (b) A/B patch 030 (bf16-act forces fp16-scale↔bf16-act mix) — suspect scale-dtype mismatch in fuse; (c) confirm gate/up interleave order matches kernel split. **NOTE: deep kernel-correctness work is at the edge of useful agentic help — expect slower, hypothesis-by-hypothesis progress here.** Then: regen patch files 001/003/004/011 for v0.5.12; gemma re-probe with skip_special_tokens=false.
