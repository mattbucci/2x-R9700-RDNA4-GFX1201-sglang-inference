...
4. **Nemotron-3-Nano-Omni-30B-A3B-Reasoning-FP8** (user-requested add) — NVIDIA Mamba2-Transformer hybrid MoE, ModelOpt per-tensor FP8, 256K-native, Omni (text+image+video+audio). **Serve no-spec** (no in-ckpt MTP — Nano excluded; no published/transferable EAGLE3/DFlash; NGRAM is CUDA-only). Pre-flight: download ~31GB, verify `mamba_ssm`/`causal_conv1d` on gfx1201, do NOT use `--attention-backend triton`; flags `--reasoning-parser nemotron_3 --tool-call-parser qwen3_coder --kv-cache-dtype fp8_e4m3`. Then 256K no-spec bench + capability validate. Task #8.
