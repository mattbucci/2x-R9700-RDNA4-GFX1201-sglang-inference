...
Next:
0. **FP8 lane kickoff (2026-05-27, R9700-owned per team split).** gfx1201 native FP8 accel → FP8 W8A8 should beat int4-AWQ-on-Triton on speed at higher quality. Recipe `scripts/quantize/quantize_fp8.py` (FP8_DYNAMIC, no calib corpus; ignores lm_head/vision/DeltaNet), env `fp8-quant` (llmcompressor 0.10). First target Devstral-24B (pure dense → dense FP8 path, lower risk than fp8-MoE kernel). **Status 2026-05-27: FP8 lane PROVEN end-to-end.** Patch 039 fixes the stock RDNA4 native fallback `_native_dynamic_per_token_quant_fp8` (didn't honor num_token_padding → `size a(17)≠b(6)`; only RDNA4 hits the native path, 3090 has vLLM kernel). Devstral-24B-FP8: code+tool PASS, **fits 256K** (TP2/mem0.88/fp8-KV → 413K-token budget, 30.7GB/card). Context math: FP8=1B/param (2× int4) → dense weights/card≈params/2 (24B=12, 31B=15.5, 32B=16) + ~10GB/card fp8-KV@256K ⇒ all dense+A3B-MoE ships still reach 256K; only >40B-dense would force a cut. Building FP8 pairs for the int4 ships next (BF16 base per model). **Build note (was blocked):** Devstral-24B FP8 built (24GB compressed-tensors float-quantized, FP8_DYNAMIC). Build needs GPU `device_map=auto` with capped `max_memory` (26GiB/card) — CPU path unsupported by llmcompressor dispatcher, uncapped GPU OOM-segfaults the 32GB cards mid-cast. Tokenizer: copy from shipped AWQ (Devstral-2507 tekken `[ARGS]` OOV breaks BPE init). **Serving blocker:** `fp8_kernel.py:1722 _native_dynamic_per_token_quant_fp8` (our patch-005 RDNA4 fallback) → `RuntimeError: size of tensor a (17) must match b (6) at dim 0` — per-token act-quant shape bug. Fix that fallback's reshape/broadcast next, then validate end-to-end before expanding to MoE ships. Audit (2026-05-27): no other CT-mislabels local; VL-REAP-26B has 0 vision tensors (structural, needs vision-preserving rebuild — 3090 lane); Coder-Next-REAM 202 flags incl. layers 43-47 (recal candidate — 3090 lane).
