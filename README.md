...
In priority order:
0. **MoE-coder gibberish — moe_wna16 EXONERATED, RC is upstream.** 8-expert grouped fused_experts_impl matches eager ref ratio 0.99997; expert0/scales/zeros/topk/repack/dtype/backend/num_stages/chat-template all clean (verifiers in scripts/debug/). Live trace: hidden absmax stays ~0.4 (small) → MoE adds ~nothing → single-token collapse. So RC is **upstream of MoE** in the 0.5.10→0.5.12 window: attn/rope/norm. **Coder uses YaRN scaling (262K)** + transformers-v5 rope_parameters change — prime suspect (dump rope cos/sin + attn out absmax per layer). Then: regen patch files 001/003/004/011 for v0.5.12; gemma re-probe skip_special_tokens=false.
