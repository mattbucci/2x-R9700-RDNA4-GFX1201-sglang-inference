...
**Next steps:**
1. **✅ RESOLVED (2026-05-31, patch 045): dense compressed-tensors FP8 OOM.** It was **NOT** a `get_scheme` dispatch bug — that diagnosis was a red herring (the CT "non-quantized schemes → UnquantizedLinearMethod" warning fires only for the intentionally-ignored DeltaNet `in_proj`; `get_scheme` correctly picks `CompressedTensorsW8A8Fp8` for all real linears, confirmed by serve-time instrumentation). **Real cause:** `qwen3_5.py:_should_replicate_deltanet_for_tp()` gated MLP TP-splitting on `name not in ("fp8","mxfp8","w8a8_fp8")`, and compressed-tensors FP8 reports `get_name()=="compressed_tensors"` → it replicated the full ~17 GB FP8 MLP on every TP rank → 28.7 GB/card → OOM. Patch 045 TP-splits the MLP for CT-FP8 too (DeltaNet `in_proj` stays replicated). **Qwen3.6-27B-FP8 now serves native FP8 @ 20.82 GB/card (was 28.70), basic+thinking+vision 3/3 PASS.** Same fix covers qwen35-27b-fp8 (same `qwen3_5` model file); gemma4-31b-fp8 is a different model class — verify separately. **Follow-up DONE (2026-05-31):** re-benched — **true single-user 256K validated** (245760-tok prefill, 9.3 tok/s, coherent) once `--chunked-prefill 2048` shrinks the per-chunk FP8 fallback-GEMM transient (auto-applied in launch.sh for the dense qwen3_5 FP8 family; A3B-MoE FP8 keeps 8192). The old 34K cap was the replication bug; FP8 dense-27B now matches AWQ-int4's 256K. No core GEMM surgery needed.
2. **✅ DONE (2026-05-31): FP8 @ 256K re-measurement campaign** (post-iommu=pt fix). The headline for single-user 256K in FP8: **only the two A3B-MoE models reach true 256K no-spec** (Coder-30B ~18 tok/s, Qwen3.6-35B ~18 tok/s, 1.6M-tok KV); **only Coder-30B also reaches 256K *with* a draft** (EAGLE3, 86 tok/s coherent). Qwen3.6-35B DFlash is short-ctx only (OOMs @256K in FP8 → AWQ-int4 is the 256K-spec path). Dense/SWA cap below 256K in FP8: Devstral-2 ~131K, gemma-4-26B ~32–64K (torch_native attn OOM wall). Capabilities re-validated 3/3 where applicable. Charts + tables updated; raw in [`benchmarks/fp8-256k-campaign-2026-05-31.json`](benchmarks/fp8-256k-campaign-2026-05-31.json).
3. **gemma-4-26B-A4B-FP8 long-context attention** — `torch_native` (only SWA-safe backend on RDNA4) lacks FlashAttention, so global-attn layers OOM past ~32–64K prefill. A chunked-KV / FlashAttention-style SWA backend for gfx1201 would unlock gemma 256K. Also fix the preset: `chunked-prefill` must be ≤ sliding_window (1024) or torch_native crashes on first long prefill.
4. **✅ Nemotron-3-Nano-Omni-30B-A3B-Reasoning-FP8 SERVES on RDNA4 (2026-05-31, user-requested add)** — NVIDIA Mamba2-Transformer hybrid MoE, ModelOpt per-tensor FP8, full AVLM (text+image+video+audio), thinking. Unblocked by **patches 043** (`is_hip` Triton causal_conv1d for `MambaMixer2` — the first Mamba2-hybrid to run on this box) **+ 044** (`modelopt_fp8` ROCm quant-allowlist) + `pip install librosa` (Parakeet sound encoder). Boots + generates coherently but **only at ≤~8K context** (`--quantization modelopt_fp8 --attention-backend torch_native --reasoning-parser nemotron_3 --kv-cache-dtype fp8_e4m3 --trust-remote-code`, TP2). KV pool 3.49M tokens (Mamba state-based — memory is NOT the limiter). **Serve no-spec** (no MTP/draft; NGRAM CUDA-only). ⚠ **Long context blocked at ~13K tokens** — the Triton-AMD SSD chunk-scan compiler bug fires at ~100+ SSD chunks (8K=64 chunks OK; ~13K=~105 chunks crash). `--chunked-prefill-size` does NOT help (the chunk-scan kernel compiles its grid over the full cumulative sequence, not the prefill sub-piece — verified 2026-05-31). Real fix must target the kernel / Triton-AMD pass. See Known Issues + Task #11.
5. **Multi-token tool-name omission recovery** for devstral (`todowrite`/`webfetch` stream in pieces; current hold-back is single-token only).
6. **Re-run the fleet SWE-bench sweep** with the new fixes (May-30 results are stale).
...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS incl. fused-expert; MoE perf root-caused (native fp8 WMMA confirmed; int4 wins M=1 decode, fp8 wins M≥8).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) — they stay BF16.
   ...
   **Validated FP8 equivalents (TP2, fp8 KV).** Coder-30B-A3B / Qwen3.6-35B-A3B / Devstral-Small-2 / gemma-4-26B were **RE-MEASURED 2026-05-31 (post-iommu=pt)** — full per-model detail in [`benchmarks/fp8-256k-campaign-2026-05-31.json`](benchmarks/fp8-256k-campaign-2026-05-31.json). Note `max ctx` = largest *single-sequence* context that actually serves (NOT the KV-pool size — they differ sharply for SWA/dense). Other rows are 2026-05-27 values, pending re-measure.
   ...
   | Qwen3.5-27B | DeltaNet hybrid | thinking PASS | 13.1 / ~26 | **256K** ✓ (patch 045 + cp2048) | **was 34K — that was the replicated-MLP OOM bug.** Patch 045 TP-splits the FP8 MLP → native FP8 20.82 GB/card; chunked-prefill 2048 (auto in launch.sh) clears the fallback-GEMM prefill OOM → **full 245760-tok prefill @mem0.90, 9.3 tok/s, validated** |
   | Qwen3.6-27B | DeltaNet+attn VL | basic+thinking+VISION 3/3 PASS | 12.8 / ~24 | **256K** ✓ (patch 045 + cp2048) | same fix as Qwen3.5-27B (shared `qwen3_5` model file); native FP8 20.82 GB/card, thinking+vision intact, true 256K prefill validated (9.3 tok/s) |
   | Qwen3.6-35B-A3B | MoE 256e (FUSED) + DeltaNet VL | basic+thinking+VISION 3/3 PASS | ~18 @32K / ~21.6 | **256K** ✓ no-spec **+ DFlash@256K** | **FP8 + DFlash now reaches 256K** (~45 tok/s coherent, accept ~3.9) via `--chunked-prefill 2048` — the prior "OOMs@256K" was a prefill-transient (KV pool 782K was never the limit), not a draft wall; launch.sh auto-applies cp2048 for qwen36-moe FP8+spec. 2nd FP8+draft@256K model |
   ...
   *Re-measured 2026-05-31 (post-iommu=pt) from [`benchmarks/fp8-256k-campaign-2026-05-31.json`](benchmarks/fp8-256k-campaign-2026-05-31.json) via `scripts/bench/generate_charts.py`. **Decode bars** are the short-ctx 2026-05-27 reference (not re-measured — keeps the FP8-vs-AWQ comparison at one context). **Max-context panel** is the corrected single-sequence 256K reachability: in FP8 only the two **A3B-MoE** models (Coder-30B, Qwen3.6-35B) reach true single-user 256K; dense/SWA cap lower — Devstral-2 ~131K, **gemma-4-26B ~32–64K** (the torch_native OOM wall, so gemma is NOT a long-context win — the earlier "lone fp8-WMMA decode win" held only at short ctx). **Green + draft** = FP8-target spec-decode: only **Coder-30B + EAGLE3 = 86 tok/s reaches 256K** (tiny 361 MB draft, pure MoE); Qwen3.6-35B + DFlash **also reaches 256K** (~45 tok/s coherent, accept ~3.9) via `--chunked-prefill 2048` — the prior "OOMs@256K" was a prefill-transient, not a draft wall (KV pool 782K was never the limit). Capabilities re-validated for all four (Coder basic; Qwen3.6-35B & gemma basic+thinking+vision 3/3; Devstral-2 basic+vision+tool). The headline: **for single-user 256K in FP8, the A3B-MoE pair wins no-spec, AND both win with a draft** (Coder-30B EAGLE3 ~86 tok/s, Qwen3.6-35B DFlash ~45 tok/s — both via `--chunked-prefill 2048`). The dense DeltaNet-VL 27B FP8 also reaches 256K no-spec (patch 045).*
   ...
   **Findings (corrects the earlier optimistic context-math):** (1) **Dense ≤24B (Devstral) is the clean FP8 win** — par speed, fits 256K. (2) **FP8 weights are 2× int4, so max context DROPS for larger/hybrid models near the 32GB limit**: 32B-dense → ~159K, 31B-dense(Gemma SWA) → ~51K, 27B-DeltaNet (mamba-state cache eats VRAM) → ~34K. AWQ-int4 reaches 262K on all of these. So FP8 *costs* context above ~24B — NOT "all dense reach 256K". (3) **A3B-MoE keeps a huge KV budget** (coder-30B 522K, qwen36-35B 1.75M tokens) — only 3B active, weights/card modest. (4) Vision + thinking + DeltaNet all survive FP8 (towers/SSM-projections kept BF16). (5) **FP8 MoE vs AWQ is an M-crossover, not a flat 2× loss** — int4 wins single-token decode (M=1) by ~1.5× (bandwidth: 0.5 B/wt vs 1 B), FP8 wins M≥8 (native gfx1201 fp8 WMMA beats int4 dequant→bf16). See the perf analysis below.
   ...
   📉 **FP8 long-context memory levers — tested 2026-05-30 (RDNA4 floor reached).** For the non-fitters (VL-32B 159K, gemma-31B 51K, 27B 34K @mem0.85): (1) **`--mem-fraction 0.92` on text-only long-context = a free +22%** — VL-32B-FP8 159K→**195K** (weights 16.9 GB/card, KV 11.9, ~2.5 spare); use it as the long-context override (keep the 0.85 default for vision+batch headroom). (2) **`--cpu-offload-gb` would reach 256K but is BROKEN for FP8 on RDNA4** — offloading 8 GB grew the KV pool to 330K tokens, but the model crashes at first inference: `RuntimeError: Only multiplication of row-major and column-major matrices is supported by cuBLASLt` in `fp8_utils._apply_fallback_scaled_mm` (the streamed-back fp8 weight layout breaks hipBLASLt; candidate future fix = force a column-major/contiguous operand). (3) **Sub-fp8 KV is unavailable on RDNA4** — `fp8_e4m3` is the floor; `fp4_e2m1` KV (the lever that would ~2× everyone's context) is CUDA 12.8+ only. (4) 27B-DeltaNet is weight-bound by mandatory-BF16 parts (`in_proj`/vision/embed), so mem-frac barely moves it. **Net: true 256K for the >24B dense/hybrid models is NOT reachable in FP8 on 2×32 GB — AWQ-int4 (half the weight bytes, `in_proj` stays BF16) remains their 256K format; FP8's clean 256K wins stay the A3B-MoE models. `--mem-fraction 0.92` is the one free squeeze.**
   ...
   **Two perf levers tested & ruled out (2026-05-27), single-user decode coder-30B/qwen36-35B FP8 baseline ≈ 17.7 tok/s @8K:**
   ...
   - **MTP speculative decode (NEXTN) — ✅ 2× decode win, the cure for "fp8 loses single-user decode".** qwen36-35B ships a 1-layer MTP head (`mtp_num_hidden_layers=1`); SGLang maps `Qwen3_5MoeForConditionalGeneration`→`Qwen3_5ForCausalLMMTP` draft. **The catch: the MTP head must stay BF16.** First attempt FP8-quantized the MTP layer → accept length ≈ 1.0, accept rate ≈ 0 (decode 17.7→15, *worse*) — the documented Qwen3-Next failure (*"mtp.fc stays INT4 → 0% MTP acceptance"*; a 1-layer next-token predictor is far more quant-sensitive than the backbone). **Fix: the caster now keeps the whole `mtp.*` block BF16** (`mtp\..*` in the ignore set + an early-copy guard, since `is_fused_expert()` would otherwise FP8 the MTP's 3D experts). Re-cast → with the SGLang-recommended config `--speculative-algorithm NEXTN --speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4`: **accept length 2.26, decode 34.5 tok/s (vs 17.7 no-spec — 2×), thinking preserved** (chain topk=1; tree-tuned to **45.25 tok/s** at `--speculative-eagle-topk 8 --speculative-num-steps 5 --speculative-num-draft-tokens 16`, accept 3.25, **4/4 validator PASS incl. VISION** under spec-decode — the 35B's mamba/DeltaNet memory caps it at topk≈8; topk=16 OOMs). That **beats the AWQ-int4 decode (~30)** — MTP lifts effective M into the regime where native fp8 WMMA wins, exactly the crossover. (SGLang auto-disables radix cache for spec+this arch on ROCm; harmless. The BF16 MTP head costs ~0.7 GB — one tiny layer.)

   - **🏁 Spec-decode summary (2026-05-28) — 7 production ships sped up at TP2.** The unlock this session was **`--speculative-attention-mode decode`**: it fixes the TP2 deadlock for EAGLE3+`moe_wna16` AND is the difference that makes DFlash win — without it spec-decode either hangs at TP2 or loses. With it, the recipe is uniform: **`<draft> + --speculative-attention-mode decode @ TP2`**. Ships & wins (all ~4× over the ~22 tok/s no-spec baseline unless noted):
     ...
     **Three reusable findings:** (1) **drafts TRANSFER parent→pruned-child** (REAP/REAM keep arch/layers/hidden — reuse the parent's draft, no recast/download/SpecForge); (2) **AWQ ships have NO MTP head** (dropped in CT→AWQ conversion — NEXTN needs grafting, so external/transferred drafts are the path); (3) **limits** — big external drafts OOM VRAM-tight DeltaNet (27B dense 3.3 GB draft OOMs even @16K); DeltaNet+draft caps context (Coder-Next-REAM 64K). *REAM-A3B vision **FIXED 2026-05-28** (was a ship defect — the vision-tower weights were absent; spliced from the parent BF16 → 4/4 PASS, serve the `-vision` dir). Still blocked: gemma (transformers+frozen-KV bug), VL-REAP (no vision tensors — structural REAP strip, needs rebuild), Qwen3-VL-32B (sgl #17935), dense-27B (draft too big).
   ...
   **Spec-decode coverage map (per model) — "working params" + draft source.** The EAGLE verify path works on RDNA4 (it's what NEXTN resolves to). **NGRAM does NOT** — it needs `sgl_kernel.reconstruct_indices_from_tree_mask`, absent from our RDNA4 sgl_kernel build (crashes on first inference). **Tree drafting (topk>1) is the throughput lever — RDNA4 supports EAGLE tree-verify** (unlike NGRAM). Start `--speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4` (chain), then widen topk for big gains: Coder-30B EAGLE3 went 54→65→81→**97 tok/s** at topk 1→4→8→16 (accept 3.4→6.0; topk=32 collapses). Pure-MoE (Coder) tolerates topk=16/draft=32; DeltaNet-MoE (35B) caps at topk≈8/draft=16 (mamba cache memory; topk=16 OOMs). **Two cardinal rules:** (1) the draft/MTP head must stay full-precision (BF16) — quantizing it collapses acceptance to ~0; (2) for an external BF16 draft against a quantized target, pass **`--speculative-draft-model-quantization unquant`** or SGLang tries to load the draft as the target's quant format and crashes (`CompressedTensorsConfig.__init__() missing ... arguments`).
   ...
   |---|---|---|---|
   | Qwen3.6-35B-A3B | in-ckpt MTP (FP8) / **DFlash (AWQ ship)** | self (NEXTN) / `z-lab/Qwen3.6-35B-A3B-DFlash` | FP8+MTP: 34.5→tuned 45 tok/s. **✅ AWQ PRODUCTION ship (2026-05-28): DFlash + `--speculative-attention-mode decode` @ TP2 = 21.8→80 tok/s (3.7×, accept 4.85), full 256K, 4/4 validator PASS (basic+thinking+vision+video).** The AWQ ship's bundled MTP head is int4 (dead → ~0 accept), so DFlash is the path — and with decode-mode it **beats the FP8 MTP tree (45) and the old FP8 DFlash (36)**. Recipe: `MODEL=…/Qwen3.6-35B-A3B-AWQ NCCL_DEBUG=WARN EXTRA_ARGS="--speculative-algorithm DFLASH --speculative-draft-model-path …/Qwen3.6-35B-A3B-DFlash --speculative-draft-model-quantization unquant --speculative-attention-mode decode --disable-overlap-schedule" scripts/launch.sh qwen36-moe`. |
   | **Qwen3-Coder-30B-A3B** | EAGLE3 (external) | `lmsys/SGLang-EAGLE3-Qwen3-Coder-30B-A3B-Instruct-SpecForge` (361 MB) | ✅ **54.2 tok/s (3×, chain) → tree-tuned 96.9 tok/s (5.4×, accept 6.0)** at `--speculative-eagle-topk 16 --speculative-num-steps 6 --speculative-num-draft-tokens 32` on FP8 target. Needs `…-quantization unquant`. (topk=32 collapses — draft drifts at depth.) **✅ Validated on the AWQ/`moe_wna16` PRODUCTION ship at full 256K (2026-05-28): TP2 + EAGLE3 + `--speculative-attention-mode decode` = ~22→97 tok/s (4.4×, accept ~5.4), coherent, context_len=262144 with max_total=817979 (huge KV headroom).** The same EAGLE3 draft (target-agnostic) lifts the int4 production model, not just FP8 — and reaches the mandate's 256K. (Use the `…-AWQ-native` dir — `quant_method=awq`. The plain `…-AWQ` dir is a complete 16 GB model but in *compressed-tensors* format, which crashes the `moe_wna16` preset; the two dirs are mislabeled vs the HF naming convention.) **THE KEY FLAG: `--speculative-attention-mode decode`.** Without it (default `prefill`), EAGLE3+`moe_wna16` at TP2 **deadlocks** — server boots, warmup passes, then a real request makes *zero* forward progress (a true hang, not just slow). With `decode` mode the hang is gone and **TP2 beats TP1** (97 vs 72–80) because spec-decode's batched verify (M≈draft-tree) likes TP2's compute parallelism, unlike no-spec M=1 where TP1 won (27 vs 22). Production recipe (TP2 is the launch.sh default; `decode` mode is the load-bearing addition): `MODEL=…/Qwen3-Coder-30B-A3B-AWQ-native NCCL_DEBUG=WARN _ENV_CTX=262144 _ENV_MAX_RUNNING=8 EXTRA_ARGS="--speculative-algorithm EAGLE3 --speculative-draft-model-path …/EAGLE3-Coder-30B-A3B --speculative-draft-model-quantization unquant --speculative-num-steps 6 --speculative-eagle-topk 16 --speculative-num-draft-tokens 32 --speculative-attention-mode decode" scripts/launch.sh coder-30b`. (TP1 also works — 72–80 tok/s, caps ~131K on one card — but TP2+decode-mode is strictly better and reaches 256K.) |
   | Qwen3.5-27B / Qwen3.6-27B (**dense** DeltaNet-VL) | in-ckpt MTP | self (NEXTN) | ⚠️ **re-cast done (BF16 MTP + DeltaNet `in_proj` fix) but FP8 spec-decode WON'T FIT on 2×32 GB.** Weights load ~28.7 GB/card: FP8 main weights stay fp8 (~13.5/card) but the *mandatory-BF16* parts replicate per-card — DeltaNet `in_proj` (192 tensors, BF16 by SSM rule) + vision tower (333) + embed/lm_head ≈ 15 GB/card. Leaves ~2.8 GB → no room for the draft (and barely serves no-spec). The old 27B-FP8 "fit 34K" only because it *wrongly* FP8'd `in_proj`. **Tried to fit FP8 by FP8'ing `in_proj`+`lm_head`** (the `--quantize-also` caster flag): footprint dropped 28.7→24.95 GB/card and with `--max-mamba-cache-size 1` + a 1/1/2 spec tree it **did fit and ran at 24.4 tok/s / accept 1.96** — **but the output was degenerate garbage** ("the the the"), no-spec *and* spec. So **FP8'ing DeltaNet `in_proj` breaks the model — the SSM "in_proj stays BF16" rule holds for FP8 too, not just INT4.** (lm_head FP8 freed ~0 — already vocab-sharded.) Net: correct cast won't even **start** no-spec on 2×32 GB (`max_running_request is zero` even at ctx 4096 — weights+working-set > 32 GB/card), and the only cast that fits is garbage → **no viable FP8 dense-27B-DeltaNet on 2×32 GB at all. AWQ-int4 (half the main-weight bytes, keeps `in_proj` BF16) is the correct format** — spec-decode for these should go via AWQ-int4 + BF16-MTP re-quant. |
   | **Qwen3-Coder REAP/REAM family** (pruned from Coder-30B) | **parent's EAGLE3 draft (TRANSFERS across pruning)** | `EAGLE3-Coder-30B-A3B` (the Coder-30B draft, reused as-is) | ✅ **2026-05-28: all three pruned text-Coder children sped up by the parent's draft (no recast/download/SpecForge):** Coder-**REAP-25B** 22.6→**90.6** tok/s (4.0×, accept 5.16, 256K); Coder-30B-**REAP** **89.5** tok/s (accept 5.14); Coder-30B-**REAM** **101.4** tok/s (accept 5.46, full 256K). REAP/REAM prune/merge experts but keep arch (`Qwen3MoeForCausalLM`), layers (48), hidden_size (2048) IDENTICAL → the parent's EAGLE3 draft predicts the child's hidden states with high acceptance. All ~4× over the family's ~22 tok/s no-spec baseline, all coherent (basic validator PASS). Recipe: `MODEL=…/<pruned-ship> EXTRA_ARGS="--speculative-algorithm EAGLE3 --speculative-draft-model-path …/EAGLE3-Coder-30B-A3B --speculative-draft-model-quantization unquant --speculative-num-steps 6 --speculative-eagle-topk 16 --speculative-num-draft-tokens 32 --speculative-attention-mode decode" scripts/launch.sh coder-30b` (use `_ENV_CTX=262144` for 256K). |
   | Qwen3.5-28B-REAP, Qwen3.6-REAM-A3B, Qwen3.6-VL-REAP-26B (**MoE**) | parent draft transfer (preferred) / in-ckpt MTP | parent's DFlash/EAGLE3 draft | **CORRECTION (2026-05-28): the AWQ ships do NOT have int4 MTP heads — the MTP layer was DROPPED entirely in CT→AWQ conversion** (config says `mtp_num_hidden_layers=1` but only layers 0..N-1 exist; layer N absent). So NEXTN can't load a head → enabling it needs *grafting* the BF16 MTP layer from the upstream base (expensive). **Better path proven on Coder-REAP-25B: reuse the PARENT model's draft.** **Qwen3.6-REAM-A3B + Qwen3.6-35B-A3B-DFlash (parent transfer) tested 2026-05-28: text/thinking = 133 tok/s median (231 max), accept 7.49 (!), basic+thinking validator PASS.** **CORRECTION (bisected 2026-05-28): DFlash+vision WORKS (the parent 35B passed vision under DFlash in #33). The REAM image request HSAIL'd because the REAM ship had NO vision-tower weights** — 0 `model.visual.*` tensors (the text-only build dropped them) while the config still declares the VL arch, so SGLang builds an unweighted tower → NaN logits on image even no-spec. **The prior "236 zero-heavy expert scales → runtime NaN" / "vision scales are clean" reading was wrong**: there were no vision scales to be clean, and the zero-heavy expert scales are benign for text *and* vision (the experts were always fine — text/thinking hits 133 tok/s). **FIXED 2026-05-28** by splicing the parent BF16's 333-tensor `model.visual.*` tower into the int4 ship (`scripts/quantize/merge_vision_weights.py --vision-prefix model.visual`) → `Qwen3.6-REAM-A3B-AWQ-vision`, **4/4 validator PASS incl. vision+video**. No re-merge or recalibration was needed. So REAM text/thinking spec-decode AND vision both ship now. For OTHER vision MoE ships with intact vision, DFlash+vision spec-decode is viable (35B proven). 28B-REAP←Qwen3.5-35B-A3B-DFlash (download; check its vision is intact first). VL-REAP still blocked by the VL spec-decode bug (#17935). |
   | Qwen3-Coder-Next | spec draft (external) | `togethercomputer/Aurora-Spec-Qwen3-Coder-Next-FP8` | untested (expected OK — EAGLE path) |
   | gemma-4-31B / 26B | assistant MTP drafter (external) | `google/gemma-4-31B-it-assistant` (927 MB, official, 4-layer shared-KV, ~3× lossless). Params: `--speculative-algorithm NEXTN --speculative-num-steps 5 --speculative-num-draft-tokens 6 --speculative-eagle-topk 1` (+ `--tp 2`) | ⚠️ **blocked by TWO layers (tested 2026-05-27).** (1) transformers: `Gemma4AssistantConfig` landed in transformers **5.8.x** (5.7.0 built 04-28 lacks it; 5.8.1 built 05-13 has it); our SGLang hard-pins `transformers==5.6.0`. Bumping to 5.8.1 is clean+revertible (only transformers changes) and **fixes config recognition + draft weight load** (`Gemma4AssistantForCausalLM` loads, NEXTN→FROZEN_KV_MTP). (2) **but SGLang v0.5.12 then crashes at the verify step** — `frozen_kv_mtp_worker.py:733 assert seq_lens.shape==extend_prefix_lens.shape`, `extend_prefix_lens is None`. So gemma spec-decode needs *both* a transformers bump **and** a SGLang frozen-KV-MTP fix/upgrade. Reverted to 5.6.0 (validated pin) since the bump alone doesn't deliver it. |
   ...
   **Web-sourced options for the blocked models (researched + partly tested 2026-05-27):**
   - **gemma-4-31B — STRUCTURALLY UNBLOCKED.** The broken assistant/FROZEN_KV_MTP path is *not* the only route: standard **EAGLE3 works** — `--attention-backend triton` (the preset's "Triton crashes on gemma SWA" note is **stale** in v0.5.12) + `--speculative-algorithm EAGLE3`. Tested gemma-4-31B-FP8 + `thoughtworks/Gemma-4-31B-Eagle3` (`LlamaForCausalLMEagle3`): loads, **coherent** ('Paris'), no `get_embed_and_head` crash. BUT accept only **1.34** (~no speedup) because that draft targets the *base* `gemma-4-31B`; the it-matched draft `RedHatAI/gemma-4-31B-it-speculator.eagle3` is `Eagle3DraftModel` arch which **SGLang can't load** (only `LlamaForCausalLMEagle3`). Remaining gap = an it-matched SGLang-format draft (convert RedHatAI's, or SpecForge-train one).
   - **Qwen3.5-27B (dense) — DFlash does NOT fit (tested 2026-05-28, OOM even at 16K).** `z-lab/Qwen3.5-27B-DFlash` is a **3.3 GB** draft (4× the 35B's 0.9 GB). On 2×32 GB the AWQ-27B weights load at 14.5 GB/card (16.98 GB free), but the 3.3 GB draft + the DeltaNet **mamba-state caches for BOTH target and draft** + a minimal KV pool overflow `pool_configurator` at every context down to 16K. No-spec 27B-AWQ fits 262K (DeltaNet KV is small); the large draft is the killer. **Redirect: the 27B's own in-checkpoint MTP head, BF16-recast, is the right path** — a ~0.7 GB head (like the 35B's) fits where the 3.3 GB DFlash draft can't, then NEXTN + `--speculative-attention-mode decode` (the proven recipe). So dense-27B spec-decode is blocked on the BF16-MTP recast, NOT on the algorithm. DFlash's big drafts suit VRAM-roomy targets, not the already-tight DeltaNet dense models.
   ...
   - **Qwen3-VL-32B** — still blocked: SGLang VL spec-decode bug (#17935, missing `get_embed_and_head` on the VL wrapper) not confirmed fixed.

   **DFlash (block-diffusion drafter) — UPGRADED 2026-05-28: with `--speculative-attention-mode decode` it's now the BEST spec-decode for the AWQ 35B ship (80 tok/s).** `z-lab/*-DFlash` drafts exist for almost the whole zoo (Coder-30B, Coder-Next, Qwen3.6-27B, **Qwen3.6-35B-A3B**, gemma-4-31B/26B-it, Qwen3.5-27B/35B-A3B). SGLang's `DFLASH` algo auto-selects the **triton** draft backend on ROCm (no NVIDIA deps). DFlash-capable target classes (implement `set_dflash_layers_to_capture`): `qwen3_moe`, `qwen3_5`, `qwen3_next`, `qwen3_vl`, `qwen3`, `llama`, deepseek_v2, gpt_oss, kimi — **but NOT `gemma4`** (it has the EAGLE3 hook, not the DFLASH one → gemma DFlash errors `does not implement set_dflash_layers_to_capture`). **The 2026-05-27 verdict "DFlash loses to tuned trees" was measured WITHOUT decode-mode and on FP8** (36.1 tok/s / accept 2.39, slower than the FP8 MTP tree's 45). **Re-tested 2026-05-28 on the AWQ PRODUCTION 35B + `--speculative-attention-mode decode` @ TP2: 80 tok/s median (145 max), accept 4.85, 4/4 validator PASS, full 256K — DFlash now WINS** (vs FP8-MTP 45, FP8-DFlash 36). decode-mode is the difference (the same flag that unblocked the Coder-30B AWQ EAGLE3 TP2 deadlock). DFlash is especially valuable for AWQ MoE ships whose bundled int4 MTP head is dead (~0 accept). Remaining DFlash gaps: gemma can't use it (no hook); Qwen3-VL-32B has the hook but no published z-lab DFlash draft.
1. **Rebuild broken & under-calibrated MoE ships** — see task list. Two ships are confirmed broken (Coder-30B-REAP-AWQ outputs gibberish — random-init experts from pre-monkey-patch REAM merge; Qwen3.6-VL-REAP-26B-A3B-AWQ HSAILs on vision — no vision tower keys). Three more have rare-expert zero scales that today's `moe_calibrate_all_experts=True` recipe fix (commit `3662f05`) should clear on recal. Tasks #22 (Coder-30B-REAP rebuild), #24 (VL-REAP rebuild), #34 (Coder-30B-REAM recal), #35 (Coder-Next-REAM recal). **#36/#39 (Qwen3.6-REAM-A3B vision) RESOLVED 2026-05-28 — vision-tower splice, not recal** (the experts were never the problem; see FP8/spec-decode section).
2. **HIP MoE kernel wiring (#28) — DONE/NEGATIVE (2026-05-28).** Wired `awq_gemv_moe_hip` into `moe_wna16.py`; served coherent at TP2 but **22.1 vs 22.3 tok/s Triton (noise)** — the MoE call is a small fraction of single-token decode, so the 2.3× *kernel-isolated* win is invisible end-to-end, and it ~2×es expert VRAM. **Reverted to Triton.** Lesson: kernel-isolated microbench ≠ end-to-end; serve-validate. Detail in `project_hip_moe_wiring` memory.
...
|---|:---:|:---:|:---:|
| `Qwen/Qwen3.6-35B-A3B` (256 exp, multimodal) | ✅ `Qwen3.6-35B-A3B-AWQ` (in-house) | ✅ `Qwen3.6-REAM-A3B-AWQ` (in-house, Samsung SAIL on upstream BF16) — **vision restored 2026-05-28** by splicing the parent's tower (4/4 PASS); merge drops `model.visual.*`, splice re-adds it | ⚠ `VL-REAP-26B-A3B-AWQ` calibrated on atbender pre-pruned BF16 (vision tower stripped at pre-prune) — rebuild from `Qwen/Qwen3.6-VL-30B-A3B-Instruct` upstream, task #24 |
...
- **Nemotron-3-Nano-Omni long-context blocked at ~13K tokens — Triton-AMD SSD chunk-scan compiler bug.** Serves + coherent at ≤~8K (patches 043/044 + librosa), but crashes the scheduler once the sequence goes multi-chunk past ~100 SSD chunks: `_chunk_scan_fwd_kernel` (`mamba/ops/ssd_chunk_scan.py`) fails in `make_ttgir` → `TritonAMDGPUCanonicalizePointers` assertion `fatPtrs.at(then)==fatPtrs.at(else)` ("expected then/else fat ptr canNarrow equal") → `PassManager::run failed` → SIGQUIT. **Boundary (verified 2026-05-31): 8192 tokens (64 chunks) = OK; ~13350 tokens (~105 chunks) = crash.** The conv1d path (patch 043) runs clean; memory is NOT the limiter (KV pool 3.49M tok). **`--chunked-prefill-size` does NOT help** — the chunk-scan kernel compiles its grid over the FULL cumulative sequence's chunk-count, not the prefill sub-piece (chunked-prefill 4096/2048 both crashed identically). The mamba `chunk_size` is already 128 (minimal). Real fix must target the kernel / Triton-AMD pass: disable `TritonAMDGPUCanonicalizePointers` for this kernel (the divergent-fat-ptr `scf.if` is the trip), patch the kernel to avoid it, or a Triton-AMD upgrade. (NB: `bench_serving --dataset-name random` is BROKEN for this Omni model — injects an image + ~236 text tokens regardless of `--random-input`; use real long-text prompts to bench.) Task #11.
...
| Qwen3.6-REAM-A3B AWQ | MoE+DeltaNet (192 exp, REAM prune of 35B), VL | 262K | 21.8 | **20.0 @131K** | `MODEL=...REAM-A3B-AWQ-vision launch.sh qwen36-moe` | Working — **vision FIXED 2026-05-28** (4/4 PASS basic+thinking+vision+video). Root cause was the vision-tower weights being *absent* (the text-only build dropped all `model.visual.*`), not zero-scale experts; spliced the 333-tensor tower back from the parent BF16 (`merge_vision_weights.py --vision-prefix model.visual`). Serve dir: `Qwen3.6-REAM-A3B-AWQ-vision`. |
...
| Qwen3.6-REAM-A3B-AWQ | ✅ | ✅ | ✅ | **vision FIXED 2026-05-28 → 4/4 PASS** (basic+thinking+vision+video) on the `-vision` dir. The earlier "text-only — REAM dropped vision tower" was correct about the *symptom* but the cause was simply **absent `model.visual.*` weights** (0 of them in the ship), not a structural REAM limitation — splicing the parent BF16's 333-tensor tower restores full multimodal on the int4 ship (no re-merge / recal needed). |
...
**Spec-decode (AWQ + draft) cross-stack — measured 2026-05-29, same probe both sides** (`merge_intervals`, 256 tok, temp 0.3; receipt [`benchmarks/quality/specdec-vs-3090-2026-05-29.json`](benchmarks/quality/specdec-vs-3090-2026-05-29.json)):
