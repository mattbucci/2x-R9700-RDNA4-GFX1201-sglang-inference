...
- **Two launch presets broken on TP1 small-ctx probes (2026-05-26).** `gemma4` hardcoded `--tokenizer-path .../gemma-4-26B-A4B-it-BF16` (reference dir, no longer on disk) → crashloop; fixed to fall back to the AWQ dir's bundled tokenizer. `qwen36-moe` ignores `CTX=2048 TP=1` overrides and crashloops at 256K/0.85-mem on one card — use TP2 or `--context-length` explicitly for single-card smoke. Both are preset bugs. **MAJOR (2026-05-26): gemma-4-26B (128exp) gibberish TOO** — `'-f(n):-n(n):-n(n)'`, 0 unbound, same repeat as coder. So it is **NOT coder-`qwen3_moe`-specific — it's the shared moe_wna16 AWQ path, regressed across MoE families** (v0.5.12 bump touched moe_wna16 silu/gelu + fused_moe). 35B `qwen3_5_moe` under test. Refocus dequant bisect on moe_wna16 itself, not coder combine.
- **3090 cross-team (2026-05-26): two CT bugs untangled, go native AWQ.** (1) `28B-REAP KeyError experts.w2_qweight` EXONERATED — the same per-expert-unfused AWQ checkpoint (`experts.N.{gate,up,down}_proj.{qweight,qzeros,scales}`, `model.language_model.` prefix) boots + **4/4 PASS basic+thinking+vision+video** on 3090 v0.5.12; **no fuse-convert needed**, patch 028's per-expert binding maps it fine. Our prior "missing" was just a stale HF-cache symlink hash. (2) qwen36 CT bombs on v0.5.12 with `KeyError: experts.w2_weight_packed` (distinct from the `_load_w2` narrow crash above): patch 028's `per_expert_match` maps AWQ suffixes (`qweight/qzeros/scales`) but NOT CT's `weight_packed/weight_scale`, so CT MoE experts never bind. Resolution both stacks: stop serving CT MoE — rebuilding qwen36 from BF16 → native AWQ-Marlin (kills the "-AWQ-CT" misnomer + 144 rare-expert flags) via `convert_moe_ct_to_awq` sym→zp=8. Updating shared `mattbucci/Qwen3.6-35B-A3B-AWQ`.
