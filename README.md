...
- **Thinking tokens (Qwen3.5)** — AWQ quantization breaks thinking stop signal. Model enters infinite `<think>` reasoning loop (4096+ tokens without `</think>`). The `<think>` tags themselves work, but the model can't decide when to stop thinking. Fix: `enable_thinking=False` for clean answers. Root cause: calibration data had no thinking traces — all existing Qwen3 AWQ quants on HuggingFace use `neuralmagic/LLM_compression_calibration` (Open-Platypus, no reasoning). Re-calibrating with thinking-aware data (e.g., `glaiveai/reasoning-v1-20m` or `a-m-team/AM-Thinking-v1-Distilled`) should preserve thinking.
...
- **Calibration quality** — Current AWQ models were calibrated with text-only Open-Platypus data. For full-quality models, calibration should include: (1) thinking/reasoning traces with `<think>` tags, (2) multimodal image+text examples for vision models, (3) domain-matched data (NuminaMath for math, code for coding models). Recommended datasets: `glaiveai/reasoning-v1-20m` (native `<think>` tags, 22M rows), `a-m-team/AM-Thinking-v1-Distilled` (Qwen3-generated, verified), `AI-MO/NuminaMath-CoT` (9.81% accuracy gain over WikiText2 for GPTQ).
