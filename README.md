...
### Active work (2026-05-09)

In priority order:

1. **Recalibrate flagged AWQ MoE builds at NUM_SAMPLES=1024** — Qwen3.6-REAM-A3B v2 SHIPPED 2026-05-10 (45.4h GPTQ + ~5 min compress + ~1h HF push). **Audit verdict: 118 flags, identical count and layer distribution as v1 — all 116 in layer 0, 2 in layer 1.** This is structural to the REAM expert-merge in the very first transformer block, not fixable by larger NUM_SAMPLES (memory `project_ream_a3b_l0_flags_structural.md`). Smoke 2/2 PASS (basic + thinking, finish=stop, answer correct). Conclusion: more samples ≠ fewer rare-expert flags for REAM MoE; future rare-expert improvement needs different REAM merger knobs, not bigger calibration corpora. Other 4 flagged builds (canonical Qwen3.6-35B-A3B-AWQ, Qwen3.6-VL-REAP-26B, Qwen3-Coder-REAP-25B, Qwen3-Coder-Next-REAM) **deprioritized in favor of #25 env rebuild** — re-evaluate need after v0.5.11 retest. Bonus from this run: `convert_moe_ct_to_awq.py` now auto-rescues text-only-recipe-on-multimodal-base saves (commit `e6191e0`) so future REAM/Coder ships skip a 30-min manual config surgery step.
2. **Mgehre HIP wvSplitK MoE kernel (#17)** — bench-first decision (#26) gates wiring. Recovered `kernels/awq_hip/awq_gemv_hip.cu:868 awq_gemv_moe_hip` (commit `c85281a`, restored as `patches/006-rdna4-awq-hip-kernels.patch` in commit `f4ffcb6`) is in-tree but unwired — `MoeWNA16Method.runner` is hardcoded `MoeRunnerBackend.TRITON`. Microbench `scripts/bench/bench_moe_hip_vs_triton.py` ready to run post-recal. HIP path needs ≥2 calls + silu+mul vs Triton 1-fused; only worth wiring if microbench shows HIP×2+silu < Triton×1 fused.
3. **In-house REAM/REAP rebuilds for 3rd-party-base ships** — multi-week. Per the build-from-scratch rule extension, three currently-shipped models source from 3rd-party prunes (Coder-REAP-25B = Cerebras, Qwen3.5-28B-REAP = Cerebras, VL-REAP-26B = atbender). Rebuild via `scripts/quantize/run_ream_qwen3moe.sh` for REAM and Cerebras's published REAP tool (`github.com/CerebrasResearch/reap`, `experiments/pruning-cli.sh`) for REAP. Sequencing: #22 Coder-30B first (text-only proof) → #24 VL-REAP (highest user value, restores broken vision) → #23 Qwen3.5-35B (Cerebras REAP-28B works fine, lowest urgency). Per-task invocations in memory `project_cerebras_reap_tool.md`. Keep current versions live until in-house validates.
4. **256K context sweeps for newly-shipped models** (Qwen3.5-28B-A3B-REAP-AWQ, Qwen3-VL-32B-AWQ, Qwen3-Coder-30B-A3B-REAM-AWQ) — primary target. Blocked by recal (no benches during calibration). Tasks #19/#20/#21.
5. **Env rebuild to v0.5.11 stack** (#25, post-recal). Drops torch 2.12 nightly → torch 2.11 stable + sglang-kernel 0.4.2 + transformers 5.6 + flashinfer 0.6.8.post1 + compressed-tensors 0.15. Expected wins on rebuild: (a) **qwen36 long-context regression closed** — 3090 TP=2/v0.5.11 shows flat 30+ tok/s 1K-250K (was 33→2.6 cratering); patch 011 already in our stack so lift transfers. (b) **Gemma 4 vision unblocked across BOTH formats** (CT + per-expert AWQ) via 3090 patch 023 detection upgrade (commit `e5e2391`) + patch 028 multimodal per-expert loader. Port both. Heads-up: 3090 patch 031 captures three v0.5.11 `qwen3_5.py` regressions (DeltaNet `in_proj_ba` quant_config carve-out, packed weight loader attr extension, `_get_split_sizes_for_param` packed-factor div) — port verbatim or qwen35/qwen36 AWQ ships break.
...
- **Gemma 4 vision degraded on v0.5.10 (RESOLVED on v0.5.11 cross-stack, R9700 verification pending env rebuild).** Pre-v0.5.11, `mattbucci/gemma-4-26B-AWQ` returned "scattered red and black pixels" on the red-circle probe instead of "a red circle". 3090 confirmed BOTH formats (CT and per-expert AWQ) lift on v0.5.11 with patch 023 detection upgrade (commit `e5e2391`) + patch 028 multimodal per-expert loader. Post env-rebuild (task #25): port both patches and re-validate.
...
- **5 audit-flagged AWQ MoE builds (rare-expert under-cal at NUM_SAMPLES=256).** `Qwen3.6-35B-A3B-AWQ` (l1.exp.214), `Qwen3.6-REAM-A3B-AWQ` (l1.exp.159), `Qwen3.6-VL-REAP-26B-A3B-AWQ` (l1.exp.166), `Qwen3-Coder-REAP-25B-A3B-AWQ` (l1.exp.22 gate AND up), `Qwen3-Coder-Next-REAM-AWQ` (l47.exp.81). Single rare expert per layer 1 with `up_proj`/`gate_proj` at 50-83% zero scales. ~1-3% degraded tokens on rare-routed paths; not serving-blocker. Recals at NUM_SAMPLES=1024 in flight (Active item #1).
- **v0.5.10 → v0.5.11 env rebuild surfaces 3 `qwen3_5.py` regressions** (cross-stack; 3090 confirmed 2026-05-09 commit `a6dc8f1`). When R9700 upgrades, port 3090 `patches/031-qwen3_5-deltanet-awq-weight-loader.patch` verbatim or every Qwen3.5/3.6 AWQ ship breaks (DeltaNet `in_proj_ba` quant_config carve-out + packed weight loader attr extension + `_get_split_sizes_for_param` packed-factor div). Affects qwen35/qwen35-moe/qwen36-tp1/qwen36-moe.
