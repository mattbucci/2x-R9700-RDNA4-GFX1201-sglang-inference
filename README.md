...
| North-Mini-Code-1.0-FP8 (`cohere2_moe`) | **6/9 django (66.7%)** ⚠ subset | First agentic validation of the 2026-06-14 parser grafts (force_rope + `cohere_command4` reasoning + tool-call) — the thinking coder drives real edits end-to-end. **⚠ Not the 15-instance set above:** ran first-15 Lite, scored only the 9 django (astropy excluded — no-docker-unscoreable), so NOT apples-to-apples with the rows above (their buildable-15 had ~3 django). Rollout 14/15 non-empty patches; scored 6 resolved / 7 applied / 9; 2 apply-fails (django-11049 over-generated a 58 KB patch; django-11039), 1 applied-but-partial (11019). Strong standalone signal; the full 15-instance buildable-subset run is the apples-to-apples follow-up. Raw: `evals/swebench/runs/north-mini-15/`. |
