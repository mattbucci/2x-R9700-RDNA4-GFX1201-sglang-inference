...
2. **Wire HIP MoE kernel into SGLang MoeRunner (#28)** — kernel + microbench landed (patches 006 + 032); runner registration in progress. **Bench-first gate PASSED (2026-05-28):** on Qwen3.6-35B-A3B-REAM dims (E=192,H=2048,I=512,top8) HIP MoE is **~2.2–2.5× faster than untuned Triton int4** at every M — awq_gemv 76.5µs vs Triton 173.7 at M=1 (2.27×, decode); wvSplitK wins M≥4 (132/215/339µs vs Triton 331/487/738 at M=4/8/16). awq_gemv best at M=1, wvSplitK best at M≥4 → hybrid dispatch (the `SGLANG_MOE_HYBRID_W4A16_MAX_BATCH=5` cutoff from patch 032). `hybrid_w4a16_moe_apply()` + skinny-reshuffle helpers exist (`layers/moe/hybrid_w4a16_moe.py`); remaining = call it from `MoeWNA16Method.apply()` (currently Triton-only) + reshuffle weights to skinny at load.
