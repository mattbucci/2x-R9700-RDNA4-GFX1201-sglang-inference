...
3. **North-Mini-Code-1.0 follow-ups** — the FP8 ship is live (`launch.sh north-mini`; in the model + sweep tables). ✅ (a) **`force_rope`→`mlp_layer_types` fix applied 2026-06-14** (live in `/data/vG` + captured in the candidate): the dense L0 was reading the popped `first_k_dense_replace` (always 0 post `__post_init__`) so `force_rope` was always False → L0 (a full-attention dense-prefix layer) ran **NoPE** instead of RoPE. Now reads `config.mlp_layer_types[i]=="dense"` at *both* the MoE-selection and `force_rope` sites (matches the 3090's patch-042 fix). Remaining: (b) graft the `cohere_command4` tool/reasoning parser; (c) agentic-bench vs the Coder-30B / REAP fleet. Candidate `patches/051-cohere2moe-model-graft.patch.CANDIDATE`.
