...
gfx1201 has native FP8 acceleration, so FP8 W8A8 is a serving option alongside the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) stay BF16.
   ...
   **Validated FP8 equivalents (TP2, fp8 KV)** — full per-model detail in [`benchmarks/fp8-256k-campaign-2026-05-31.json`](benchmarks/fp8-256k-campaign-2026-05-31.json). `max ctx` = largest *single-sequence* context that actually serves (NOT the KV-pool size — they differ sharply for SWA/dense).

   | FP8 model | type | quality | tok/s (FP8/AWQ) | max ctx @mem0.85 | notes |
   |---|---|---|:---:|:---:|---|
   | Devstral-24B (v1) | dense | code+tool PASS | 37 / 37 | **256K** (413K-tok KV) | text-only Devstral-Small-2507; FP8 par w/ AWQ |
   ...
   | Qwen3-Coder-30B-A3B | MoE 128e | code PASS | ~18 @32K / ~30 | **256K** (522K-tok KV) | **FP8+EAGLE3 = 86 tok/s coherent @256K** (tiny 361MB draft, pure MoE, accept ~5.5); no-spec flat 18/17 @32K/131K |
   | Qwen3-VL-32B | dense VL | basic+VISION PASS | 13.0 / — | ~159K (158K tok) | largest dense; vision survives FP8 |
   | gemma-4-31B | dense (Gemma4) | basic+thinking PASS | 11.6 / ~15 | ~51K (51K tok) | native FP8 (17.57 GB/card, no Linear fallback); cap is memory; vision = known AWQ HSAIL (not FP8) |
...
| Gemma 4 26B AWQ | MoE (128 experts) | 256K | 30 | — | `launch.sh gemma4` | Working (3/4 validate: basic+thinking+vision PASS, video FAIL — `vision.py:254 assert bsz==1` triggered by 12-frame video) |
| Gemma 4 31B AWQ | Dense | 256K | 15 | — | `launch.sh gemma4-31b` | Working (torch_native) |
| Qwen3.5-27B AWQ | DeltaNet hybrid | 262K | 26 | 14 @65K | `launch.sh qwen35` | Working (thinking-aware) |
...
| Qwen3.6-35B MoE AWQ | MoE+DeltaNet (256 experts) | 262K | 21.6 | 20.6 @131K | `launch.sh qwen36-moe` | Working — `mattbucci/Qwen3.6-35B-A3B-AWQ`, 256K basic+thinking+vision PASS |
| Qwen3.6-27B AWQ | DeltaNet+attn hybrid (VL) | 262K | 24.1 | 9.8 @131K | `launch.sh qwen36-27b` | Working (native AWQ); 64 layers in 3:1 linear/full pattern |
| Coder-REAP-25B AWQ | MoE (96 exp, REAP prune of Coder-30B) | 256K | 22.9 | **21.9 @131K** | `launch.sh coder-reap-25b` | Working (self-calibrated code_thinking + native AWQ) |
| Qwen3.6-REAM-A3B AWQ | MoE+DeltaNet (192 exp, REAM prune of 35B), VL | 262K | 21.8 | **20.0 @131K** | `MODEL=...REAM-A3B-AWQ-vision launch.sh qwen36-moe` | Working — 4/4 PASS basic+thinking+vision+video. The text-only build drops all `model.visual.*`, so serve the `-vision` dir (333-tensor tower spliced back from the parent BF16 via `merge_vision_weights.py --vision-prefix model.visual`). |
| Nemotron-3-Nano-Omni-30B-A3B FP8 | Mamba2-Transformer hybrid MoE, AVLM | 256K | 29 | 29 @247K | `launch.sh nemotron-omni` | Working — NVIDIA ModelOpt FP8; full AVLM (text+image+video+audio) + thinking, first Mamba2 hybrid on the box. Triton flash attention; requires patches 043/044/046/047 + `pip install librosa`. No spec-decode (no published draft). |
