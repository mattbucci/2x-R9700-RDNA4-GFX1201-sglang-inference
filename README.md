...
- **AWQ scales audit needs the base to judge zero-scales (3090 2026-05-31)** — MoE bases ship *dead* channels (`Qwen3.6-35B-A3B`: 50-72% of some layer-0 expert gate/up channels at ~7.8e-38), and AWQ faithfully underflows their fp16 scale to 0 — a benign dead channel, not the v2 dequant-to-zero→NaN bug. `check_awq_scales.py --base <bf16_dir>` downgrades those dead-block zeros while keeping zero-over-*live* as `DEFECT` (qwen36: 144 benign flags → 0 residual; injected live-block zero still caught). **Two gotchas if you port it** (both fixed in 3090 `a2b54dc`, found via a fleet-wide audit): (1) dead channels come in *two magnitudes* — Qwen3.6 denormals (~1e-38) AND Coder-30B REAP/REAM near-zeros (~1e-26); a `DEAD_THRESH` of 1e-30 misreads the 1e-26 ones as live → false `DEFECT` on clean ships. Use **1e-15** (sits in the >20-order gap below live ~1e-2; no false negatives). (2) Map *both* base layouts — fused `experts.gate_up_proj` (Qwen3.5/3.6) **and** unfused per-expert `experts.{e}.gate_proj.weight` (Qwen3Moe/Coder), else Coder flags stay unresolved. Port `--base` — your `qwen36-moe`/`qwen35-moe`/Coder-REAP/REAM ships all need it. 3090 `59db82c`+`a2b54dc`.
- **int4 agentic "0/6 ceiling" looks like an int4 × triton-attention interaction, NOT an int4 capability ceiling — re-test with `torch_native` attention IN the agentic harness (3090 2026-06-05).** Cross-stack triangulation, SWE-bench Lite opencode, same v0.5.12 stack: **int4 + FlashInfer (3090) is clean** — `qwen36-moe` (MoE-thinking) **59.0%**, `qwen36-ream` **58.7%** (both *fleet-leading*, above the coders); Devstral (dense, no-think, int4) emitted **0 garbled tool calls across 262K calls incl. 86.8K @64-128K**. Versus your **int4 + triton = 0/6** and **fp8 + triton = 4/6**. Removing *either* int4 *or* triton rescues it → the garbling (your xarray 71-invalid@82-steps) reads as the **patch-011 triton BF16 precision loss "over many KV tokens" compounding int4's noisier high-entropy tool-format logits** — it only bites at long KV. **The `e772509` investigation ruled out attention via *single-turn* probes, but single-turn never reaches the many-KV regime where 011 itself says the error compounds; the agentic failures are all long-KV.** Decisive test (isolates attention, holds model+quant fixed): re-run the int4 6-instance smoke with `--attention-backend torch_native` (your reference-quality path) *inside* the agentic harness, not single-turn. If 0→nonzero, it's attention precision — fix = the triton-attn rekernel (011 is partial / "insufficient for deep pipelines"), and int4 keeps its M=1/256K throughput edge for agentic; no FP8 fallback needed. Bisection tool ported: `scripts/eval/context_reliability_curve.py` buckets tool-call garble-rate (`tool=="invalid"`) by TRUE per-step `tokens.input` — run it across {triton, torch_native} × {int4, fp8} × {fp8-KV, fp16-KV} and watch where the garble-vs-ctx curve flattens. Honest confound: our 59% is MoE-thinking vs your dense Qwen3.5, so the model differs — which is *why* the same-model `torch_native` A/B is the clean isolator. Same method also showed the *resolve*-vs-context decline is **universal/hardness** (coders fall steeper than thinking), not quant. 3090 receipts `benchmarks/quality/context-reliability-2026-06-05.{md,json}`, commit `da1a58e`; full narrative in [CLAUDE.md](CLAUDE.md#cross-team-lessons-full-narrative).
- **cuda-graph-ON confirmation for pure-attention MoE (answering your quick-check, 3090 2026-06-06).** Your "confirm pure-attention MoE presets run cuda-graph ON" — on the 3090 (32K ctx, decode): **Qwen3-Coder-30B MoE runs cuda-graph ON → `cuda graph: True` in decode, 196 tok/s** (`--cuda-graph-max-bs 1` + `--disable-piecewise-cuda-graph` — full graph on, piecewise off because TP-path awq_marlin MoE piecewise regresses; matches your 2.3-2.5× win, so ✅ confirmed there). **But `gemma4` (Gemma-4-26B MoE) runs cuda-graph OFF → `cuda graph: False`, 36 tok/s** — NOT a perf choice: Gemma's SWA + multimodal force `--attention-backend triton` on sm_86, and cuda-graph can't capture that path (same triton-attention constraint as patch-011 above). So the ~5× decode gap (196 vs 36) on our side is the *Gemma triton backend*, not the MoE arch — if Gemma text-only could ride FlashInfer, cuda-graph would land. Worth checking whether your gfx1201 Gemma path eats the same cuda-graph-off penalty. 3090 launch.sh per-preset CUDA_GRAPH flags; empirical via decode-log `cuda graph:` lines.
- **Gemma-4-12B *unified* (`Gemma4UnifiedForConditionalGeneration`, the encoder-free omni 12B) needs its arch name added to THREE hybrid-SWA lists or TP>1 crashes on first prefill — `store_cache: expected 2 got 1` (3090 2026-06-07).** Same mixed per-layer-type KV as the 26B/31B — sliding layers `(head_dim=256, kv=8)`, global layers `(head_dim=512, kv=1)` — but its arch string differs from the 26B's `Gemma4ForConditionalGeneration`, so it silently falls out of `configs/model_config.py`'s `is_hybrid_swa_model` (→ uniform MHA pool, one head_dim=512, can't store the sliding layers' `4*256=1024`-wide k), `get_hybrid_layer_ids`, and `is_hybrid_swa_compress` (the flag that makes the kv-cache mixin pass `swa_head_num`/`swa_head_dim` to the SWA sub-pool). Fix = add the arch to all three (3090 patch 047, one-liner each). NOT Ampere-specific — if you serve the 12B on gfx1201 at TP=2 you'll hit the identical crash; the SWA-pool path is shared SGLang core. The model is the most KV-efficient Gemma (no SigLIP tower; BF16 only ~24 GB) but BF16 KV still caps ~47K @TP=2, so int4 is required for long context.
- **QAT-unquantized bases → AWQ by data-free RTN-from-safetensors; skip calibration AND the model load entirely (3090 2026-06-07).** Google ships `google/gemma-4-12B-it-qat-q4_0-unquantized` (QAT-trained, weights already on a 4-bit grid). Re-quantizing to AWQ group-128 is near-lossless **without any calibration corpus** — MLP layers cos=1.00000 vs base, attn projs 0.993-0.996. And you don't need transformers to *recognize the arch* (ours 5.5.4 doesn't know `gemma4_unified`): pack straight from the safetensors tensors with AutoAWQ's `WQLinear_GEMM.from_linear` (pass `scales`/`zeros` from `AwqQuantizer.pseudo_quantize_tensor`, **transpose to `[in/G, out]`** — `from_linear` indexes the group along dim0). Quantize only the text-decoder Linears; keep vision/audio embedders + norms + lm_head BF16. `check_awq_scales` 0/328, serves awq_marlin, text 4096 quality == BF16. So when an arch is too new for your llmcompressor/transformers but a QAT base exists, this dodges the whole "vendor the modeling code into the quant env" problem. 3090 recipe `scripts/quantize/quantize_gemma4_12b_qat_rtn_awq.py`.
- **...but RTN-from-QAT only WINS where there's no strong prior AWQ — on already-well-calibrated bases it just TIES (3090 2026-06-07).** We rebuilt all three Gemma 4 sizes from their QAT bases and evaluated against the shipped AWQs: **12B = clear win → shipped** (there was no good prior AWQ); **31B (dense VL) = tie → kept the shipped GPTQ-based AWQ**; **26B (A4B MoE, 128 exp) = tie → kept shipped** (MMLU 23/30 vs 24/30 = one-question noise, HE 14/20 vs 15/15). The hoped-for "QAT conditions EVERY expert → beats GPTQ's rare-expert under-calibration gap" did **not** materialize on the 26B MoE — a well-calibrated GPTQ/AWQ ship is already at the same ceiling, so faithful 4-bit-grid re-quant has nothing left to win. Two MoE-specific notes for R9700 if you try this: (1) the 26B fused experts are bare `nn.Parameter` (`experts.gate_up_proj [E,2*inter,hidden]` / `experts.down_proj [E,hidden,inter]`, **no `.weight` suffix**) — unfuse per-expert before packing; (2) `experts.down_proj` in-features=704 isn't /128, so **group_size MUST be 32**, which means the per-expert AWQ isn't Marlin-packable and SGLang correctly falls back to the **WNA16 MoE kernel** (boots clean — not a failure). Net: reach for QAT-RTN to *create* an AWQ for an arch that has none; don't expect it to *beat* an existing calibrated ship. 3090 recipe `scripts/quantize/quantize_gemma4_moe_qat_rtn_awq.py`, receipts `benchmarks/quality/gemma4-26b-qat.json`.
- **The DeltaNet+MoE hybrids (qwen36 family) were needlessly running cuda-graph OFF — enabling it is a 4× single-user decode win, and their triton GDN kernels DO capture (3090 2026-06-07).** Extends the cuda-graph bullet above: our `qwen36` + `qwen36-ream` (Qwen3.6-35B/REAM-A3B, DeltaNet+MoE) carried a stale `--disable-cuda-graph`. Tell-tale: **TPOT flat at ~33 ms from 1K to 262K** — a decode step is `const-forward + attention-over-KV`, so the attention term *must* grow with context (the graph-ON `qwen3-ream` shows 5.5→9.5 ms); a flat line means a fixed ~27 ms/step *kernel-launch* overhead is swamping everything, i.e. no graph. Enabling `--cuda-graph-max-bs 1 --disable-piecewise-cuda-graph` (single-user capture; piecewise off for the awq_marlin-MoE TP regression) drops TPOT 32→8 ms: **controlled A/B @262K = 31 → 126 (qwen36) / 139 (qwen36-ream) tok/s, 4.1–4.5×**; @1K 31→174. **5/5 capabilities under graph replay** (basic+tool+thinking+vision+video) on both — DeltaNet recurrent state is correct captured. **Key refinement to the Gemma cuda-graph-OFF bullet above:** qwen36's DeltaNet layers run **triton GDN kernels** (`TritonGDNKernel` decode/extend/verify) and *still* graph-captured cleanly — so "triton ⇒ can't graph-capture" is too broad; it's specifically Gemma's SWA *softmax*-attention triton path that won't capture, not triton GDN. **Two gotchas:** (1) capture succeeds but the server then died at the *final* init step with `avail 2.9 GB` (parent broken-pipe, no kernel OOM) — needed `--mem-fraction 0.85→0.80` headroom for graph+warmup (KV pool still ~876K/2.17M ≫ 262K); (2) match the captured bs to single-user (`MAX_RUNNING=1`/bs=1) or just cap the graph at bs=1. **For gfx1201:** check whether your `qwen36-moe` / `qwen36-27b` / `qwen35-moe` presets carry `--disable-cuda-graph` and whether HIP-graph capture works for the hybrid on ROCm — if it does, it's the same ~4× single-user 256K decode win. Corollary we hit: this also moots DFlash spec-decode on qwen36 (its "4.1×" was vs the graph-OFF 31 baseline; graph-ON no-spec already matches it). 3090 commit `9531a1a`, per-preset `CUDA_GRAPH` flags + `_ENV_CUDA_GRAPH` override hook in `scripts/launch.sh`.
- **HumanEval `exec()` of model-generated code needs a subprocess + hard timeout — one pathological generation GIL-spun our whole fleet eval for 4h (3090 2026-06-08).** Our `eval_and_chart.py` ran each HumanEval solution+test via `exec()` inside a `ThreadPoolExecutor` worker. A model emitted an `O(2^n)` `prime_fib` (a recursive-genexpr generation) that never returns; `signal.alarm` can't interrupt it (worker thread, not main), so the run wedged at 99% CPU with **zero server requests** for 4 hours — `py-spy dump` on the hung PID was what finally showed the `exec` frame, not a model/serving bug. Fix = run each solution in an **isolated subprocess** (`subprocess.run([sys.executable,"-c",script], timeout=10)`, returncode==0 == pass); a timeout fails that one item instead of hanging the harness. This *also* firmed the numbers — the pathological gen now correctly **fails** (qwen36 lands HE 39/40, not tanked-by-hang). Any fleet HumanEval harness that `exec()`s untrusted gen in-process has this landmine — check yours. 3090 commit `afaa3e7`.
...
| Coder-30B | MoE (128 experts) | 256K / 256K | 56.0 / 38.3 | `launch.sh coder-30b` | Working. FP8 522K-tok KV; **FP8+EAGLE3 = 86 tok/s coherent @256K** (361 MB draft, accept ~5.5); AWQ + EAGLE3 production = 97 tok/s @256K. **No-spec now cuda-graph ON (2026-06-01 sweep): AWQ 56.0 @short / 21.4 @131K / 10.6 @256K (2.27× over the prior eager 24.7), FP8 38.3 @short (was 14.7); see [cuda-graph note](#cuda-graph-doubles-moe-decode).** |
| Gemma 4 26B | MoE (128 experts) | 256K / 256K (triton) | 52.9 / 41.8 | `launch.sh gemma4` | Working (3/4 validate: basic+thinking+vision PASS, video FAIL — `vision.py:254 assert bsz==1` on 12-frame video). 256K (262144) via TRITON flash (validated FP8 + AWQ long prefill, needle retrieved, no OOM); gemma4 preset defaults to triton. **No-spec now cuda-graph ON (2026-06-01 sweep): AWQ 52.9 @short / 21.4 @131K / 14.3 @256K (1.66× over eager 31.9), FP8 41.8 @short (was 18.7); triton SWA flash captures fine, validate_capabilities 3/3 incl. vision under graph.** The old ~32–64K cap was torch_native-only (ROCm MATH SDPA → O(chunk×ctx) score OOM); `ATTN_BACKEND=torch_native` is the capped fallback. |
...
| Qwen3.5-35B MoE | MoE+DeltaNet (256 experts) | 262K / — | 60.7 / — | `launch.sh qwen35-moe` | Working. **AWQ no-spec now cuda-graph ON (2026-06-01 sweep): 60.7 @short / 27.8 @131K / 20.0 @256K (2.33× over eager 26.1).** DeltaNet+MoE holds long ctx best (O(1) linear-attn state). No FP8 build. |
| Qwen3.6-35B-A3B (MoE) | MoE 256e (FUSED) + DeltaNet VL | 262K / 256K | 60.2 / 36.4 | `launch.sh qwen36-moe` | Working — `mattbucci/Qwen3.6-35B-A3B-AWQ`, 256K basic+thinking+vision PASS. **No-spec now cuda-graph ON (2026-06-01 sweep): AWQ 60.2 @short / 28.1 @131K / 19.9 @256K (2.27× over eager 26.5), FP8 36.4 @short (was 15.0); ON-vs-OFF temp-0 bit-exact (DeltaNet state capture verified).** FP8 reaches 256K no-spec **+ DFlash@256K** ~45 tok/s (accept ~3.9) via `--chunked-prefill 2048` (auto for qwen36-moe FP8+spec); KV pool 782K. AWQ + DFlash production = 80 tok/s @256K. |
...
| Coder-REAP-25B | MoE (96 exp, REAP prune of Coder-30B) | 256K / — | 56.8 / — | `launch.sh coder-reap-25b` | Working (self-calibrated code_thinking + native AWQ). **AWQ no-spec now cuda-graph ON (2026-06-01 sweep): 56.8 @short / 23.7 @131K / 15.2 @256K (2.48× over eager 22.9; same pure-attention A3B MoE family as Coder-30B).** |
| Qwen3.6-REAM-A3B | MoE+DeltaNet (192 exp, REAM prune of 35B), VL | 262K / — | 58.9 / — | `MODEL=...REAM-A3B-AWQ-vision launch.sh qwen36-moe` | Working — 4/4 PASS basic+thinking+vision+video. **AWQ no-spec now cuda-graph ON (2026-06-01 sweep): 58.9 @short / 28.1 @131K / 20.0 @256K (2.70× over eager 21.8; inherits the qwen36-moe preset's cuda-graph).** Text-only build drops all `model.visual.*`, so serve the `-vision` dir (333-tensor tower spliced from parent BF16 via `merge_vision_weights.py --vision-prefix model.visual`). FP8 not built — a 256→192 FP8 re-merge needs the 67 GB parent BF16 resident, which wedges this 64 GB box; ships in AWQ. |
| Nemotron-3-Nano-Omni-30B-A3B | Mamba2-Transformer hybrid MoE, AVLM | — / 256K | — / 64 | `launch.sh nemotron-omni` | FP8-only (NVIDIA ModelOpt FP8, no AWQ variant). 256K (262144) via triton flash; full AVLM (text+image+video+audio) + thinking, first Mamba2 hybrid on the box. **cuda-graph ON (2026-06-01): 64 @short / 52 @131K / 45 @256K, ~2.0× over eager ~31 (flat) — M=1 decode is launch-bound across the whole hybrid, not compute-bound; see [cuda-graph note](#cuda-graph-doubles-moe-decode).** torch_native is a slow fallback that OOMs past ~150K (ROCm MATH SDPA only). Requires patches 043/044/046/047 + `pip install librosa`. No spec-decode (no published draft; Nano excluded from MTP). |
...
The win is largest at short ctx (the launch gap is a fixed ~21 ms while KV-read GPU time grows) and stays positive through 256K. The **DeltaNet+MoE** models hold long context best — **~20 tok/s @256K** vs pure-attention MoE 10–15 — because their linear-attention state is O(1). Crucially, **recurrent-state graph capture is numerically exact**: qwen36-moe's ON-vs-OFF temp-0 deterministic output is bit-identical (similarity 1.000), so there's no silent DeltaNet/mamba state corruption. **This is the opposite of the dense models**, which are GPU-bound (Qwen3.6-27B decode profiled at 85.8% util) — there cuda-graph has no gap to remove, which is why the global default stays `--disable-cuda-graph` and the DeltaNet *dense* presets (`qwen35`, `qwen36-27b`) keep it off. `nemotron-omni` (Mamba2-Transformer hybrid MoE, **FP8** — the first FP8 model *and* first Mamba2 hybrid captured on the box) is now **ON**: graph capture works cleanly (4.4 s, bs=[1,2,4,8]), and its cuda-graph-OFF decode curve is *flat at ~31 tok/s from 128 → 245K ctx* — the whole 52-layer hybrid (Mamba2 conv1d + SSD scan + MoE dispatch + attn) is launch-bound at M=1, **not** compute-bound, so the Mamba2 layers I'd have guessed were GPU-bound aren't. Capture lifts it to 64 @short / 45 @256K (2.0× short, 1.43× even at 256K where the eager ceiling is purely launch overhead). `coder-next-ream` (60B DeltaNet+MoE, 131K) is also now **ON**: 2.13× short but the win flattens to ~1.07× by 131K — the first model where it nearly vanishes at max ctx, because the 60B's real per-token compute (~45 ms) has climbed to *meet* the launch-overhead ceiling (no regression, so it stays on). The full 80B `coder-next` isn't downloaded; otherwise the **cuda-graph roster is complete** — every MoE/hybrid preset captures, only the GPU-bound dense presets stay eager. Toggle on any preset with `CUDA_GRAPH_ENABLE=1 launch.sh <preset>`; capture is cheap (~0.3–0.5 GB, bs=[1,2,4,8,12,16,24], ~5–8 s at boot).
