...
In priority order:
0. **MoE-coder gibberish — YaRN EXONERATED (2026-05-25): broken at 4K ctx, not just 262K.** Live REAM server @4096 ctx returns gibberish (`'n:n:nn:nn'`, `'isisisis'`) — rope scaling doesn't engage until ~32K, so YaRN/262K is NOT the cause. moe_wna16 also exonerated: 8-expert grouped fused_experts_impl ratio 1.0000. 96-expert grouped ALSO exact (ratio 0.99995, `moe_96expert_grouped_vs_ref.py`) — kernel clean at full scale. RC is full-model gating/norm/attn, not MoE GEMM. Next: live per-layer absmax trace on coder forward to find collapse layer. Then: regen patch 001/003/004/011 for v0.5.12; gemma re-probe skip_special_tokens=false.
