...
**RESOLVED (2026-04-24 night, 3090 team): Qwen3.6-35B-A3B SHIPPED on 3090 via your `convert_moe_ct_to_awq.py`.** Direct safetensors inspection of the published CT upload showed `shared_expert_gate.weight_packed` with shape `(1, H)` — output dim 1 can't be AWQ-packed (requires divisibility by 8), and the NVIDIA SGLang CT loader doesn't do the BF16 fallback your conversion script does.  Running your script (ported verbatim) unpacks the CT checkpoint, repacks the experts to native AWQ, and passes through `shared_expert_gate` as BF16 — 30970 INT4 + 40 BF16 fallbacks + 696 other passthroughs.  Post-conversion on 3090: loads clean as `Qwen3_5MoeForConditionalGeneration` with patch 019, validator **4/4 PASS** (basic/thinking/vision all green), **~33 tok/s short-ctx** (vs your 21.6 on ROCm — +55%; NVIDIA `awq_marlin` kernel is just faster at this size).  The vision tower issue from the follow-up above turned out to be a false alarm: `model-vision.safetensors` in your upload is entirely BF16 (167 `.weight` tensors, 0 packed) because llmcompressor's GPTQ only targets `Linear` and Qwen3.6's vision tower has Conv2d + non-Linear paths that skip the quant pass.  So your 35B CT upload is good-as-is for any downstream consumer that runs the CT→AWQ conversion.  Proposed upload path for convenience: `mattbucci/Qwen3.6-35B-A3B-AWQ-native-thinking-vision` so users can skip the conversion step entirely.
