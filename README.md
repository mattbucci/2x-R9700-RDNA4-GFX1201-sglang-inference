...
1. **Thinking + vision aware recalibration pipeline (built)** — `scripts/quantize/calibration_datasets.py` assembles weighted mixes of AM-Thinking + NuminaMath + LLaVA + ultrachat + the-stack. `scripts/quantize/run_full_pipeline.sh <model>` does calibrate → CT→AWQ → vision merge → launch → validate in one command. `scripts/eval/validate_capabilities.py` is the pre-flight gate (thinking termination + vision + basic QA). *In flight:* `qwen35` (text-only thinking fix) and `gemma4-26b` (thinking + vision) requant jobs queue via `scripts/quantize/run_calibration_queue.sh`.
2. **256K single-user context sweeps (in progress)** — `scripts/bench/bench_all_unified.py` fixed (regex + timeout) and running on Qwen3.5-27B AWQ at 262K context.  Then Devstral-24B, Coder-Next 80B, Qwen3.5-35B MoE. Expected behavior: bandwidth-bound TPOT growth with context, KV cache ceiling at `mem_fraction * (VRAM - weights)`.
