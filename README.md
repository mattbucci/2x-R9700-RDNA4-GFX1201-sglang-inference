...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS (perf follow-up open).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet/MoE-router (stay BF16).
   ...
   **Serving:** patch 039 fixes the RDNA4 native per-token act-quant (`_native_dynamic_per_token_quant_fp8` padding); patch 004 fixes the RDNA4 fused-MoE fallback (`_rdna4_torch_moe` hardcoded `block_m=16` ≠ real `BLOCK_SIZE_M` → walked into uninitialized `expert_ids` → garbage index). **RDNA4 FP8/INT8 MoE now defaults to the real Triton `fused_moe_kernel`** (`RDNA4_TORCH_FP8_MOE=1` opt-in restores the torch loop) — the "Triton hangs 30min" rationale was stale (predated patch 039; `scaled_fp8_quant` uses native PyTorch on HIP, no Triton compile). `launch.sh` auto-detects `compressed-tensors` in config.json and serves any int4 preset's FP8 dir via the CT path (`MODEL=<fp8-dir> launch.sh <preset>`).
   ...
   | FP8 model | type | serves 256K | quality | tok/s | vs AWQ int4 |
   |---|---|:---:|---|:---:|---|
   | Devstral-24B-FP8 | dense | ✅ (413K-tok KV budget, 30.7GB/card) | code+tool PASS | 37 | ≈ par |
   | Qwen3-Coder-30B-A3B-FP8 | MoE 128-exp | ✅ (522K-tok KV budget) | code+tool PASS | **14.7** (Triton) / 4.7 (torch fallback) | ~30 → **FP8 MoE is slower** |

   Context math: FP8=1B/param (2× int4) → dense weights/card≈params/2 (24B=12, 31B=15.5, 32B=16) + ~10GB/card fp8-KV@256K ⇒ all dense + A3B-MoE ships still reach 256K; only >40B-dense would force a cut. Confirmed: both above fit 256K with 1.6–2× KV headroom.
