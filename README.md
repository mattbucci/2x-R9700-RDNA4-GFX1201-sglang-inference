...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS incl. fused-expert; MoE perf root-caused (native fp8 WMMA confirmed; int4 wins M=1 decode, fp8 wins M≥8).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) — they stay BF16.
   ...
   **Spec-decode coverage map (per model) — "working params" + draft source.** The EAGLE verify path works on RDNA4 (it's what NEXTN resolves to). **NGRAM does NOT** — it needs `sgl_kernel.reconstruct_indices_from_tree_mask`, absent from our RDNA4 sgl_kernel build (crashes on first inference). **Tree drafting (topk>1) is the throughput lever — RDNA4 supports EAGLE tree-verify** (unlike NGRAM). Start `--speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4` (chain), then widen topk for big gains: Coder-30B EAGLE3 went 54→65→81→**97 tok/s** at topk 1→4→8→16 (accept 3.4→6.0; topk=32 collapses). Pure-MoE (Coder) tolerates topk=16/draft=32; DeltaNet-MoE (35B) caps at topk≈8/draft=16 (mamba cache memory; topk=16 OOMs). **Two cardinal rules:** (1) the draft/MTP head must stay full-precision (BF16) — quantizing it collapses acceptance to ~0; (2) for an external BF16 draft against a quantized target, pass **`--speculative-draft-model-quantization unquant`** or SGLang tries to load the draft as the target's quant format and crashes (`CompressedTensorsConfig.__init__() missing ... arguments`).
   ...
   |---|---|---|---|
   | Qwen3.6-35B-A3B | in-ckpt MTP | self (NEXTN) | ✅ **34.5 tok/s (2×), accept 2.26** — BF16 MTP head |
   ...
   | Qwen3.5-28B-REAP, Qwen3.6-REAM-A3B, Qwen3.6-VL-REAP-26B (**MoE**) | in-ckpt MTP | self (NEXTN) | recipe proven on 35B-A3B; these are AWQ ships with int4 MTP heads → need BF16-MTP re-quant. MoE FP8 stays compact (fits spec) unlike dense. |
   ...
   **Web-sourced options for the blocked models (researched + partly tested 2026-05-27):**
   ...
   - **Qwen3.5-27B (dense) — DFlash exists** (`z-lab/Qwen3.5-27B-DFlash`, SGLang `--speculative-algorithm DFLASH`). FP8 target still won't fit 2×32 GB, but **AWQ-int4-27B + DFlash** would (int4 fits 262K) — a realistic spec-decode path for the dense 27B at int4 accuracy. DFlash claims up to 5× (block-diffusion drafter).
   ...
   **DFlash (block-diffusion drafter) — works on RDNA4 but doesn't beat our tuned trees (tested 2026-05-27).** `z-lab/*-DFlash` drafts exist for almost the whole zoo (Coder-30B, Coder-Next, Qwen3.6-27B, **Qwen3.6-35B-A3B**, gemma-4-31B/26B-it, Qwen3.5-27B/35B-A3B). SGLang's `DFLASH` algo auto-selects the **triton** draft backend on ROCm (no NVIDIA deps). DFlash-capable target classes (implement `set_dflash_layers_to_capture`): `qwen3_moe`, `qwen3_5`, `qwen3_next`, `qwen3_vl`, `qwen3`, `llama`, deepseek_v2, gpt_oss, kimi — **but NOT `gemma4`** (it has the EAGLE3 hook, not the DFLASH one → gemma DFlash errors `does not implement set_dflash_layers_to_capture`). Tested **Qwen3.6-35B-A3B-FP8 + `z-lab/Qwen3.6-35B-A3B-DFlash`**: runs, coherent, **36.1 tok/s / accept 2.39** — *slower than its tuned MTP tree (45.25, accept 3.25)*. So DFlash adds no new win here: where we have a tuned MTP/EAGLE3 tree it loses; gemma can't use it (no hook); Qwen3-VL-32B has the hook but no published z-lab DFlash draft.
...
2. **Wire HIP MoE kernel into SGLang MoeRunner (#28) — DONE/NEGATIVE (2026-05-28): no end-to-end win, reverted.** Implemented the full decode wiring in `moe_wna16.py` (raw on-disk AWQ capture w/ TP-slice → `awq_gemv_moe_hip` gate_up→silu→down→reduce in `apply()`), served Qwen3-Coder-30B-A3B-AWQ-native at TP2: **output coherent** (layout + AWQ zero-points + TP-slice all CORRECT) but decode **22.09 tok/s vs 22.27 Triton — identical (within noise)**. The bench-first gate (HIP 2.2–2.5× *kernel-isolated*) was **misleading**: the MoE call is a small fraction of single-token decode (attention/norms/sampling/dispatch dominate), so a 2.27× kernel speedup is invisible end-to-end — AND keeping a raw AWQ copy alongside the converted weights ~2×es expert VRAM. **Reverted to Triton** (clean, no memory cost). Lesson: kernel-isolated microbench ≠ end-to-end; always serve-validate. Implementation + the (still-open) multi-user-batch angle captured in `project_hip_moe_wiring` memory.
