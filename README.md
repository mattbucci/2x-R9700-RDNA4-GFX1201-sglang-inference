...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS incl. fused-expert; MoE perf root-caused (native fp8 WMMA confirmed; int4 wins M=1 decode, fp8 wins M≥8).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) — they stay BF16.
   ...
   **Spec-decode coverage map (per model) — "working params" + draft source.** The EAGLE verify path works on RDNA4 (it's what NEXTN resolves to). **NGRAM does NOT** — it needs `sgl_kernel.reconstruct_indices_from_tree_mask`, absent from our RDNA4 sgl_kernel build (crashes on first inference). **Tree drafting (topk>1) is the throughput lever — RDNA4 supports EAGLE tree-verify** (unlike NGRAM). Start `--speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4` (chain), then widen topk for big gains: Coder-30B EAGLE3 went 54→65→81→**97 tok/s** at topk 1→4→8→16 (accept 3.4→6.0; topk=32 collapses). Pure-MoE (Coder) tolerates topk=16/draft=32; DeltaNet-MoE (35B) caps at topk≈8/draft=16 (mamba cache memory; topk=16 OOMs). **Two cardinal rules:** (1) the draft/MTP head must stay full-precision (BF16) — quantizing it collapses acceptance to ~0; (2) for an external BF16 draft against a quantized target, pass **`--speculative-draft-model-quantization unquant`** or SGLang tries to load the draft as the target's quant format and crashes (`CompressedTensorsConfig.__init__() missing ... arguments`).
   ...
   | **Qwen3-Coder-30B-A3B** | EAGLE3 (external) | `lmsys/SGLang-EAGLE3-Qwen3-Coder-30B-A3B-Instruct-SpecForge` (361 MB) | ✅ **54.2 tok/s (3×, chain) → tree-tuned 96.9 tok/s (5.4×, accept 6.0)** at `--speculative-eagle-topk 16 --speculative-num-steps 6 --speculative-num-draft-tokens 32` on FP8 target. Needs `…-quantization unquant`. (topk=32 collapses — draft drifts at depth.) **✅ Also validated on the AWQ/`moe_wna16` PRODUCTION ship (2026-05-28): 27.2→72–80 tok/s (2.7–3×, accept 5.2–5.4), coherent, fits 131K on ONE card.** The same EAGLE3 draft (target-agnostic) lifts the int4 production model, not just FP8. (Use the `…-AWQ-native` dir — `quant_method=awq`. The plain `…-AWQ` dir is a complete 16 GB model but in *compressed-tensors* format, which crashes the `moe_wna16` preset; the two dirs are mislabeled vs the HF naming convention.) **CRITICAL: must run TP1.** At TP2 the EAGLE3 verify path **deadlocks** on the `moe_wna16` MoE all-reduce (server boots, then zero forward progress — a real hang, not just slow). TP1 also beats TP2 *no-spec* (27.2 vs ~22) since the model fits one 32 GB card and skips all-reduce. Production recipe: `MODEL=…/Qwen3-Coder-30B-A3B-AWQ-native TP=1 CUDA_VISIBLE_DEVICES=0 NCCL_DEBUG=WARN _ENV_CTX=131072 _ENV_MEM=0.90 _ENV_MAX_RUNNING=8 EXTRA_ARGS="--speculative-algorithm EAGLE3 --speculative-draft-model-path …/EAGLE3-Coder-30B-A3B --speculative-draft-model-quantization unquant --speculative-num-steps 6 --speculative-eagle-topk 16 --speculative-num-draft-tokens 32" scripts/launch.sh coder-30b`. **Limit:** TP1 caps context at ~131K (276633 max tokens, ~2.3 GB free at 131K); full 256K needs TP2 → blocked by the deadlock. |
