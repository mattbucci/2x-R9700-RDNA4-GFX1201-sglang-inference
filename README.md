...
2. **Qwen3.6-35B v2 calibration — two ship blockers identified, v1 stays prod.** v2 (audit-fixed `re:.*shared_expert\..*` ignore) loads only after a config-class patch and then crashes on first inference because of a separate BF16-shared_expert load-side bug. Full investigation log in [patches/README.md](patches/README.md#recent-resolved-items). Open follow-up: `convert_moe_ct_to_awq.py --reference-config` flag to script the config rescue (item #4 below); proper SGLang loader patch for the BF16 shared_expert path TBD.
...
6. **REAM + REAP variant coverage for every MoE model.**  REAM and REAP are **different** expert-pruning strategies — not aliases.  Coder-Next-REAM 60B proves the REAM path works on our stack (25 tok/s @ 131K).  Cerebras's `Qwen3-Coder-REAP-25B-A3B` is a REAP prune of Coder-30B (different algorithm).  Goal: produce + bench + validate both method families per MoE model wherever public weights exist, and self-prune otherwise.  **In flight (2026-04-26):** building REAM-pruned Qwen3.6-35B-A3B (256→192 experts) ourselves via Samsung SAIL `merge.py` (CPU, c4+math+code calibration). Layer 0 took **7h12m** at default settings (`batch_size=8`, sequential mode); revised whole-merge ETA is ~12 days, not the 24-28h initially estimated. The algorithm runs many ream-grouping rounds per layer, each followed by a full propagate-hidden-states forward pass; sequential mode gates speed.  Output: `~/AI/models/Qwen3.6-35B-A3B-REAM-BF16` then quant pipeline → estimated ~27B AWQ that should fit 256K with comfortable VRAM headroom. **No mid-run checkpointing** — any crash = total loss; restart costs full elapsed time. Future REAM attempts: try `--batch_size 32` (4× forward speedup if RAM holds) and consider reducing calibration data first.  For the remaining MoE models (Gemma4-26B, Qwen3.5-35B-A3B), search HF for both method names separately.  Bench both REAM and REAP independently — don't label one as the other.
