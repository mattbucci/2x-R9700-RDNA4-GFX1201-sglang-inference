...
| Coder-30B | MoE (128 experts) | 256K / 256K | 57.6 / 14.7 | `launch.sh coder-30b` | Working. FP8 522K-tok KV; **FP8+EAGLE3 = 86 tok/s coherent @256K** (361 MB draft, accept ~5.5); AWQ + EAGLE3 production = 97 tok/s @256K. **AWQ no-spec now 57.6 @short / 23.3 @128K / 15.0 @256K (2026-06-01 cuda-graph ON — 2.33× over the prior 24.7; see [cuda-graph note](#cuda-graph-doubles-pure-attention-moe-decode)).** FP8 no-spec 14.7 (cuda-graph not yet re-measured). |
| Gemma 4 26B | MoE (128 experts) | 256K / 256K (triton) | 53.2 / 18.7 | `launch.sh gemma4` | Working (3/4 validate: basic+thinking+vision PASS, video FAIL — `vision.py:254 assert bsz==1` on 12-frame video). 256K (262144) via TRITON flash (validated FP8 + AWQ long prefill, needle retrieved, no OOM); gemma4 preset defaults to triton. **AWQ no-spec now 53.2 @short (2026-06-01 cuda-graph ON — 1.67× over 31.9; triton SWA flash captures fine, validate_capabilities 3/3 PASS incl. vision under graph).** FP8 18.7 (cuda-graph not re-measured). The old ~32–64K cap was torch_native-only (ROCm MATH SDPA → O(chunk×ctx) score OOM); `ATTN_BACKEND=torch_native` is the capped fallback. |
...
| Coder-REAP-25B | MoE (96 exp, REAP prune of Coder-30B) | 256K / — | 58.1 / — | `launch.sh coder-reap-25b` | Working (self-calibrated code_thinking + native AWQ). **AWQ no-spec now 58.1 @short (2026-06-01 cuda-graph ON — 2.54× over 22.9; same pure-attention A3B MoE family as Coder-30B).** **21.9 @131K** long-ctx (pre-cuda-graph; long-ctx gain smaller, see note). |
...
#### cuda-graph doubles pure-attention MoE decode

**cuda-graph is OFF by default on this box but ON for the pure-attention A3B MoE presets (`coder-30b`, `coder-reap-25b`, `gemma4`) as of 2026-06-01 — it ~2× their single-user decode.** Profiling `coder-30b` decode (cuda-graph OFF, conc=1, ctx 8K) showed only **~48% GPU utilization** — 40.6 ms wall/step vs **19.5 ms GPU-busy** ([receipt](benchmarks/profiling/coder-30b-decode-profile-2026-06-01.json)). The kernel-family split is `elementwise_norm 29.6% · nccl 24.4% · moe(fused_moe_gptq_awq) 20.1% · awq_gemv 9.5% · rocblas_gemm 8.6%` — i.e. the work is many *tiny* kernels (an A3B MoE activates only ~3B params/token, so the GEMMs are small while the per-layer norm/rope/router/allreduce launch count is fixed). That makes M=1 MoE decode **dispatch-bound**, and cuda-graph replay removes the ~21 ms/step launch gap:

| Model | OFF (eager) | ON (cuda-graph) | speedup | validated |
|-------|:-----------:|:---------------:|:-------:|-----------|
| Coder-30B | 24.7 | **57.6** @short / 23.3 @128K / 15.0 @256K | 2.33× short | coherent code @256K (218K-tok input) |
| Coder-REAP-25B | 22.9 | **58.1** @short | 2.54× | coherent |
| Gemma 4 26B (VL) | 31.9 | **53.2** @short | 1.67× | validate_capabilities 3/3 — basic+thinking+**vision** intact under graph |

The win shrinks at long context (the launch gap is a fixed ~21 ms while KV-read GPU time grows), so it's largest at short ctx and still positive at 256K. **This is the opposite of the dense/DeltaNet models**, which are GPU-bound (Qwen3.6-27B decode profiled at 85.8% util) — there cuda-graph has no gap to remove, which is why the global default stays `--disable-cuda-graph`. DeltaNet/Mamba presets (`qwen35-moe`, `qwen36-moe`, `coder-next`, `nemotron-omni`) are left OFF (recurrent-state graph capture untested/risky). Toggle on any preset with `CUDA_GRAPH_ENABLE=1 launch.sh <preset>`; capture is cheap (~0.3–0.5 GB, bs=[1,2,4,8,12,16,24], ~5–8 s at boot).
