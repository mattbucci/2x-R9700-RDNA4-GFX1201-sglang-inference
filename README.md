...
1. **Recalibrate flagged AWQ MoE builds at NUM_SAMPLES=1024** — IN FLIGHT (Qwen3.6-REAM-A3B with `balanced_thinking_text` recipe; ~48h ETA from start at 01:15 PDT 2026-05-09). Other 4 flagged builds (canonical Qwen3.6-35B-A3B-AWQ, Qwen3.6-VL-REAP-26B, Qwen3-Coder-REAP-25B, Qwen3-Coder-Next-REAM) queue sequentially since BF16 base + Hessian doesn't leave room for parallel runs at 62 GB host RAM. Mitigations: `max_shard_size="2GB"` on save; `re:.*shared_expert_gate.*` in ignore lists; `check_awq_scales.py` wired as ship gate.
...
|----------|:------------:|:----:|:----:|
| Qwen3.6-35B-A3B (256 exp) | ✅ | ✅ `Qwen3.6-REAM-A3B-AWQ` | ⚠️ only `VL-REAP-26B-A3B-AWQ` (atbender, vision tower stripped) — task #24 rebuild |
| Qwen3-Coder-30B-A3B (128 exp) | ✅ | ✅ `Qwen3-Coder-30B-A3B-REAM-AWQ` (in-house, 2026-05-09) | ✅ Cerebras `Coder-REAP-25B-A3B-AWQ` (rebuild #22) |
| Qwen3-Coder-Next 80B (512 exp) | (unshipped) | ✅ `Coder-Next-REAM-AWQ` (~60B effective) | ❌ — task #46 |
| Gemma 4 26B (103 exp) | ✅ `gemma-4-26B-AWQ` | ❌ no shipper | ❌ no shipper |
| Qwen3.5-35B-A3B → Cerebras REAP→28B-A3B | ❌ unshipped | ❌ no shipper | ✅ `Qwen3.5-28B-A3B-REAP-AWQ` (3090 ship 2026-05-02; rebuild #23) |
