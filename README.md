...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS incl. fused-expert; MoE perf root-caused (native fp8 WMMA confirmed; int4 wins M=1 decode, fp8 wins M≥8).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) — they stay BF16.
   ...
   *Re-measured 2026-05-31 (post-iommu=pt) from [`benchmarks/fp8-256k-campaign-2026-05-31.json`](benchmarks/fp8-256k-campaign-2026-05-31.json) via `scripts/bench/generate_charts.py`. **Decode bars** are the short-ctx 2026-05-27 reference (not re-measured — keeps the FP8-vs-AWQ comparison at one context). **Max-context panel** is the corrected single-sequence 256K reachability: in FP8 only the two **A3B-MoE** models (Coder-30B, Qwen3.6-35B) reach true single-user 256K; dense/SWA cap lower — Devstral-2 ~131K, **gemma-4-26B ~32–64K** (the torch_native OOM wall, so gemma is NOT a long-context win — the earlier "lone fp8-WMMA decode win" held only at short ctx). **Green + draft** = FP8-target spec-decode: only **Coder-30B + EAGLE3 = 86 tok/s reaches 256K** (tiny 361 MB draft, pure MoE); Qwen3.6-35B + DFlash = 57 tok/s **short-ctx only** (BF16 draft + DeltaNet prefill OOM at 256K → AWQ-int4 is the 256K-with-draft path at 80 tok/s). Capabilities re-validated for all four (Coder basic; Qwen3.6-35B & gemma basic+thinking+vision 3/3; Devstral-2 basic+vision+tool). The headline: **for single-user 256K in FP8, the A3B-MoE pair wins no-spec, and only Coder-30B also wins with a draft.***
