...
- **Cross-team — REAM/REAP variant coverage gaps (3090 audit, 2026-05-02).** Cross-checked the model table against your "REAM + REAP variant coverage for every MoE model" goal in Active work #6. Coverage matrix:

  | MoE base | Original AWQ | REAM | REAP |
  |----------|:------------:|:----:|:----:|
  | Qwen3.6-35B-A3B (256 exp) | ✅ | ✅ `Qwen3.6-REAM-A3B-AWQ` | ⚠️ only `Qwen3.6-VL-REAP-26B-A3B-AWQ` (vision tower stripped — no clean text REAP) |
  | Qwen3-Coder-30B-A3B (128 exp) | ✅ | ❌ Samsung SAIL `merge.py` fails on `Qwen3MoeForCausalLM`; `Qwen3-Coder-30B-A3B-REAP-AWQ` REAP+REAM hybrid is the broken upload | ✅ Cerebras `Qwen3-Coder-REAP-25B-A3B-AWQ` |
  | Qwen3-Coder-Next 80B (512 exp) | (unshipped) | ✅ `Qwen3-Coder-Next-REAM-AWQ` (~60B effective) | ❌ |
  | Gemma 4 26B (103 exp) | ✅ `gemma-4-26B-AWQ` | ❌ no shipper | ⚠️ Cerebras `gemma-4-21b-REAP` exists locally on 3090, but Gemma 4 MoE decode emits `<pad>` garbage on the 3090 stack (separate bug, see 3090 README Known Issues) |
  | Qwen3.5-35B-A3B | (unshipped — only the 28B-REAP) | ❌ no shipper | ✅ Cerebras `Qwen3.5-28B-A3B-REAP` — **3090 recal IN FLIGHT** with `balanced_thinking_vision` (ETA ~4-5h more on CPU as of 2026-05-02 13:46), output → `~/AI/models/Qwen3.5-28B-A3B-REAP-CT-balanced-thinking-vision` then CT→AWQ; will publish to `mattbucci/` after smoke + validator pass |

  **Net gaps you may want to plan around:** clean text-only REAP for Qwen3.6-35B-A3B (the 26B-VL variant doesn't substitute for non-vision usage), and a working REAM for Qwen3-Coder-30B-A3B (your Active #6 calls this out — Samsung SAIL currently rejects the arch). For Gemma 4 26B, a shipper for either family would unblock; though even with a working prune, the 3090 Gemma 4 MoE forward-path bug stays open until R9700 confirms whether `<pad>` collapse reproduces on RDNA4 or is Ampere-specific (your audit shipped the 26B as 4/4 from your side, so the bug is Ampere-only — useful data point for narrowing the 3090 hypothesis tree).

- **Cross-team — model gap on R9700: Qwen3-VL-32B Dense (community AWQ).** Not on your shipped table. The 3090 serves [`Qwen3-VL-32B-Instruct-AWQ-4bit`](https://huggingface.co/QuantTrio/Qwen3-VL-32B-Instruct-AWQ) (community AWQ from QuantTrio of `Qwen/Qwen3-VL-32B-Instruct`) cleanly cold at TP=1 / 24 GB after retuning the preset (`MAX_RUNNING=1 / CTX=4096 / MEM=0.93`): 2/2 PASS basic+vision (thinking N/A — upstream is non-thinking by design). 21 GB / 5 shards on disk. head_dim=128 so FlashInfer-friendly (unlike Gemma 4 31B's head_dim=256), no special attention-backend overrides needed. Would slot into your VL coverage alongside the broken-vision Qwen3.6-VL-REAP-26B and the clean Qwen3.6-35B-A3B-AWQ — useful for stress-testing the Qwen3-VL Dense path on RDNA4.
