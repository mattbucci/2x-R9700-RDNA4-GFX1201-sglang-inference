...
- **Coding-harness scaffold mismatch is the bigger story for qwen36 — same model + parser, +80pp swing between scaffolds (3090 2026-05-13).** Followup 5-instance smokes on identical astropy instances after the tool-call-parser fix: `qwen36 × claw-code = 0/5 = 0%` (model gets stuck in `<think>...</think>` silence or read-only exploration loops with `grep_search`/`read_file`/`glob_search`, never commits to source edits) vs `qwen36 × opencode = 4/5 = 80%` (real fixes including the canonical `cright[...] = right` gold patch for separability_matrix and `output_field[:] = chararray.replace(...)` gold patch for FITS D-exponent). Same model, same chat template, same `--tool-call-parser qwen3_coder`. The one unresolved opencode instance (14365 qdp.py) used the same incomplete `re.IGNORECASE` fix in both scaffolds → that's the model's understanding limit, not scaffold. Practical takeaway: **don't pick scaffolds blind to model training data** — thinking-mode generalists (Qwen3.5/3.6) match opencode's tool registry well but mismatch claw's expected iteration loop; coder-tuned models (Coder-30B, REAP-25B) match claw's `Bash`/`Edit`/`Read` tools well. The right benchmark cell is `(model_family, scaffold)`, not just `model`. Per-cell receipts: `benchmarks/quality/bakeoff-qwen36-{claw-code,opencode}-postfix-smoke.json` in 3090 repo, commit `c1e53ad`.
- **Keyword-grep capability validators miss silent fabrication (M4 2026-05-13).** M4 ported the 3090 probe trio (`probe_thinking` / `probe_vision` / `probe_codegen`) and discovered that the validator's keyword grep was passing fabricated VLM responses: `validate_capabilities.py:check_vision` for a red-circle-on-white image passed Devstral when the response was *"A diagram of a circular flow chart with a central circle labeled '1'..."* — because "circle" appeared in the text. Real recognition vs confabulation requires content-aware classification (STRONG / DEGRADED / FAIL). R9700 should treat `probe_vision.py` as the gate for every recal, not `validate_capabilities.py:check_vision`. Calibrations that fix `weight_packed` / scale flags can still ship a model whose vision tower is producing degraded features — the probe is what tells you whether the eyes are actually seeing.
