...
gfx1201 has native FP8 acceleration, so FP8 W8A8 is a serving option alongside the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) stay BF16.
...
./scripts/launch.sh qwen3vl-32b         # Qwen3-VL-32B Dense AWQ
./scripts/launch.sh nemotron-omni       # Nemotron-3-Nano-Omni-30B-A3B FP8 (Mamba2 hybrid AVLM+thinking, 256K)
...
This is the canonical per-model table — AWQ-int4 (the recommended RDNA4 runtime) and FP8 W8A8 (native gfx1201 acceleration) folded into one. `Max ctx` and `tok/s` columns show `AWQ / FP8` (short-ctx single-user decode; `—` where a format isn't built/reachable). FP8 `max ctx` = largest *single-sequence* context that actually serves (NOT the KV-pool size); full per-model FP8 detail in [`benchmarks/fp8-256k-campaign-2026-05-31.json`](benchmarks/fp8-256k-campaign-2026-05-31.json).

| Model | Type | Max ctx (AWQ / FP8) | tok/s (AWQ / FP8) | Launch | Status & notes |
|-------|------|:-------------------:|:-----------------:|:------:|----------------|
| Devstral-24B | Dense | 256K / 256K | 37 / 37 | `launch.sh devstral` | Working. Text-only Devstral-Small-2507; FP8 par with AWQ, fits full 256K (413K-tok KV) — the clean dense FP8 win. |
| Devstral-2-24B | Dense + vision | 256K / ~180K | — / ~23 @32K | `launch.sh devstral2` | v2 Mistral3 FP8 (`devstral2`): basic+vision+tool PASS. 256K not reachable in FP8 (transient floor ~12.5 GB free @KV-sizing; BF16 vision tower eats KV; pool 180572@0.90); AWQ-int4 is the dense 256K path. No draft. |
| Coder-30B | MoE (128 experts) | 256K / 256K | 30 / ~18 @32K | `launch.sh coder-30b` | Working. FP8 522K-tok KV; **FP8+EAGLE3 = 86 tok/s coherent @256K** (361 MB draft, accept ~5.5); AWQ + EAGLE3 production = 97 tok/s @256K. No-spec FP8 flat ~18/17 @32K/131K. |
| Gemma 4 26B | MoE (128 experts) | 256K / 256K (triton) | 30 / ~16.5 @110K | `launch.sh gemma4` | Working (3/4 validate: basic+thinking+vision PASS, video FAIL — `vision.py:254 assert bsz==1` on 12-frame video). 256K (262144) via TRITON flash (validated FP8 + AWQ long prefill, needle retrieved, no OOM); gemma4 preset defaults to triton. The old ~32–64K cap was torch_native-only (ROCm MATH SDPA → O(chunk×ctx) score OOM); `ATTN_BACKEND=torch_native` is the capped fallback. |
| Gemma 4 31B | Dense (Gemma4) | ~105K (triton) / ~51K | 15 / 11.6 | `launch.sh gemma4-31b` | Working — TRITON default (validated: 80K-tok prefill no OOM + needle retrieved, 718-tok decode clean — the old ~400-tok triton degradation is gone). ~105K pool ceiling @mem0.92 — dense-31B is weight/pool-bound (NOT 256K like the 26B MoE); torch_native fallback capped ~32–64K + had an HSAIL decode crash. basic+thinking PASS. FP8 build ~51K (not re-tested on triton). Vision = known AWQ HSAIL. |
| Qwen3.5-27B | DeltaNet hybrid | 262K / 256K | 26 / 13.1 | `launch.sh qwen35` | Working (thinking-aware; thinking PASS in FP8). FP8 native 20.82 GB/card; 256K needs patch 045 + chunked-prefill 2048 (auto in launch.sh) to clear the fallback-GEMM prefill OOM → full 245760-tok prefill @mem0.90, 9.3 tok/s. |
| Qwen3-VL-32B | Dense VL | — / ~159K | — / 13.0 | `launch.sh qwen3vl-32b` | FP8-only here (largest dense; basic+VISION PASS, vision survives FP8). ~159K @mem0.85 (158K tok); `--mem-fraction 0.92` lifts it to ~195K. No AWQ build in this table. |
| Coder-Next 80B | MoE+DeltaNet (512 experts) | 131K / — | 24 / — | `launch.sh coder-next` | Boots + short generates; HSAIL 0x1016 on long decode (see Known Issues). |
| Coder-Next REAM 60B | MoE+DeltaNet (384 experts) | 131K / — | 25 / — | `launch.sh coder-next-ream` | Working. |
| Qwen3.5-35B MoE | MoE+DeltaNet (256 experts) | 262K / — | 14-16 / — | `launch.sh qwen35-moe` | Working (GPTQ ship). **12.4 @256K** long-ctx. |
| Qwen3.6-35B-A3B (MoE) | MoE 256e (FUSED) + DeltaNet VL | 262K / 256K | 21.6 / ~18 @32K | `launch.sh qwen36-moe` | Working — `mattbucci/Qwen3.6-35B-A3B-AWQ`, 256K basic+thinking+vision PASS. FP8 reaches 256K no-spec **+ DFlash@256K** ~45 tok/s (accept ~3.9) via `--chunked-prefill 2048` (auto for qwen36-moe FP8+spec); FP8+MTP tuned to 45 tok/s; KV pool 782K. AWQ + DFlash production = 80 tok/s @256K. |
| Qwen3.6-27B | DeltaNet+attn hybrid (VL) | 262K / 256K | 24.1 / 12.8 | `launch.sh qwen36-27b` | Working (native AWQ); 64 layers in 3:1 linear/full pattern. FP8 same path as Qwen3.5-27B (shared `qwen3_5` file): native 20.82 GB/card, basic+thinking+VISION 3/3 PASS, 256K via patch 045 + cp2048, 9.3 tok/s @256K. |
