...
2. **Per-layer decode dispatch regressed ~1.6× in v0.5.12 — bisect the per-layer kernel-launch path.** conc=1 short-ctx decode is dispatch-bound and scales with **layer count**, not arch or quant: Devstral-2-24B (40 layers) = 24 tok/s (42 ms); Qwen3.5-27B / Qwen3.6-27B (64-layer DeltaNet hybrid) and **dense Qwen3-VL-32B (64 layers)** all = ~14–15 (66–69 ms) ≈ 1 ms/layer. Ruled out as the lever: (a) the AWQ Linear/GEMV is healthy (40-layer Devstral hits 24); (b) `--quantization awq` flag (ROCm no-op — `is_awq_marlin_compatible` gates on `_is_cuda=False`); (c) **cuda-graph — gives no decode speedup on v0.5.12, verified `cuda graph: True` on both a dense and a hybrid 64-layer model, still ~14–15**. An April-24 sweep got the 64-layer Qwen3.6-27B to **24.1 (41.5 ms, ~0.65 ms/layer)**, not reproducible now. So the remaining cost is the ~19 per-layer kernel launches/step that don't amortize under ROCm cuda-graph; the older SGLang tree paid them cheaper. Bisect old vs v0.5.12 decode dispatch (kernel fusion / why ROCm graph replay ≈ eager). Single-user lever across **every** model — bigger the more layers. (Qwen3-VL-32B gap now filled: 15.2 @128 / 9.2 @131K, 256K KV 273174, vision PASS.)
...
All values tok/s single-user, conc=1.  The Qwen3.5-27B / Qwen3.6-27B AWQ rows were **re-measured 2026-05-31** from the clean conc=1 sweep JSONLs (`sglang_0531_1_*`) + live spot-checks — the two DeltaNet-27B hybrids share one curve (~14.4 short → 13.0 @32K → 11.8 @65K → 9.7 @131K → ~7.2 @256K).  **This corrects the earlier 24–26 short-ctx figures, which the current v0.5.12 + patch-041 GEMV stack does not reproduce** (current dense/hybrid AWQ M=1 decode is ~14.4, confirmed by JSONL data + fresh runs; cuda-graph makes no difference — verified `Decode batch … cuda graph: True` still decodes ~13).  ⚠ **Open regression — decode is per-layer dispatch-bound.** Devstral-2-24B (40 layers) decodes 24 tok/s (42 ms), but every **64-layer** model — Qwen3.5/3.6-27B (DeltaNet hybrid) *and* dense **Qwen3-VL-32B** — clusters at ~14–15 (66–69 ms), i.e. ~1 ms/layer regardless of arch.  **cuda-graph gives no decode speedup on v0.5.12** (verified `cuda graph: True` on both a dense and a hybrid 64-layer model — still ~14–15).  An April-24 sweep recorded **24.1 (41.5 ms)** for the 64-layer Qwen3.6-27B (~0.65 ms/layer) — *not reproducible* now by `--quantization` flag or cuda-graph.  The lever is the per-layer decode kernel-launch overhead (~19 launches/layer, no graph benefit on ROCm); the older tree apparently paid it cheaper.  Bisect — Open work #2.  †262K from the Qwen3.6-27B 245760-tok point (7.2 tok/s, TTFT 31 s); the family is measured-identical so it applies to both rows.  Both 35B-A3B MoE models hit the 256K target with similar characteristics; Qwen3.6 edges out Qwen3.5 at 256K (13.3 vs 12.4).
