...
2. **🔄 IN PROGRESS (2026-05-31): FP8 @ 256K re-measurement campaign** — single-user decode tok/s for FP8 *no-spec* and FP8 *+ draft* (spec-decode), at full 256K, with patches 041 (AWQ M=1 GEMV) + 042 (FP8 transient reclaim) applied. Re-validating capabilities (thinking/vision/video/tool) at 256K in the same pass. Run one model at a time (only one fits at 256K) via per-model subagents; aggregate → `fp8-comparison.json` + `specdecode.json` → regenerate charts. Core 256K-capable FP8 set: Coder-30B-A3B (+EAGLE3), Qwen3.6-35B-A3B (+DFlash), Devstral-Small-2-24B (no draft), gemma-4-26B-A4B (no DFlash hook).
...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS incl. fused-expert; MoE perf root-caused (native fp8 WMMA confirmed; int4 wins M=1 decode, fp8 wins M≥8).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) — they stay BF16.
   ...
   *Chart regenerated 2026-05-29 from the FP8 lane sweep (`benchmarks/fp8-comparison.json`, `scripts/bench/generate_charts.py`). Takeaways: (a) no-spec FP8 matches AWQ decode + keeps full 256K **only** for dense ≤24B (Devstral) and the A3B-MoE models; it loses single-user MoE/DeltaNet decode (int4 wins the M=1 bandwidth race) and costs max context above ~24B (gemma-4-26B-A4B is the lone fp8-WMMA decode win). (b) The green **+ draft** bars are production spec-decode (target + draft + `--speculative-attention-mode decode` @ TP2) — and on this box **only the two A3B-MoE models have a usable draft**: Coder-30B-A3B + EAGLE3 → **97**, Qwen3.6-35B-A3B + DFlash → **80**. The dense/DeltaNet/VL/gemma models are **0 / N/A** (no published draft, the draft OOMs the DeltaNet/dense KV, gemma has no DFlash hook, or VL spec is broken — sgl#17935).*
