...
2. **Optimize the `awq_gemv_bf16_kernel` — it's 78% of decode GPU time and ~5× under memory roofline.** Profiled qwen36-27b decode (conc=1, TP0 DECODE trace, [`benchmarks/profiling/qwen36-27b-decode-profile-2026-05-31.json`](benchmarks/profiling/qwen36-27b-decode-profile-2026-05-31.json)): decode is **GPU-compute-bound — 85.8% GPU utilization** (68 ms GPU-busy of 79 ms wall/step), so it is NOT dispatch/launch-bound, which is exactly why cuda-graph gives no speedup (no gaps to remove — verified `cuda graph: True` still ~14–15 on dense *and* hybrid). The patch-041 **`awq_gemv_bf16_kernel` is the bottleneck at 78%** (~53 ms/step); DeltaNet `fused_recurrent_gated_delta` is 0.6% (5 ms — *not* the issue), NCCL all-reduce 7.7%. The decode weight-read roofline is ~10.5 ms/step (13.5 GB int4 / TP2 / ~640 GB/s GDDR7), so the GEMV runs **~5× under roofline** — a HIP-kernel efficiency problem (occupancy / vectorization / split-k / tiling), kernel at `components/sglang/sgl-kernel/csrc/quantization/awq/awq_gemv_hip.cu`. Single-user lever across **every** AWQ model; up to ~5× headroom (first ~1.7× just matches the April-24 24.1 the current GEMV doesn't reach — April predates patch-041 and likely rode better-tuned rocBLAS GEMM). Ruled out: `--quantization` flag (ROCm no-op), cuda-graph (no gaps), DeltaNet (5 ms). **Microbench** ([`scripts/bench/bench_awq_gemv.py`](scripts/bench/bench_awq_gemv.py), receipt [`benchmarks/profiling/awq_gemv_microbench-2026-05-31.json`](benchmarks/profiling/awq_gemv_microbench-2026-05-31.json), cos=1.0): per-shape **18–50% of roofline** — efficiency rises with N because `num_blocks = N/256` underfills the 64 CUs for small-N projections (attn_o N=5120 → 20 blocks → 18%; gate_up N=13824 → 54 blocks → 50%). `SPLIT_K` (warps-in-block) climbs sk1→sk16 = 2%→18%, then plateaus → a second cap (per-int4 dequant ALU + scalar 4 B `qweight` loads). **Grid-level split-K tested + reverted** ([receipt](benchmarks/profiling/awq_gemv_gridsplit-experiment-2026-05-31.json), cos=1.0): a 1-warp-per-(n-tile,k-split) block with float-atomic accumulate only helped the most occupancy-starved shape (attn_o N=5120 **1.31×**) and *regressed* the rest (gate_up 0.61×, down 0.85×) — atomics+cast overhead beats smem reduction once occupancy is adequate. So **occupancy is only a partial lever; the ~30–50% plateau is the inner loop** (dequant ALU + scalar loads). **Next lever:** vectorize `qweight` to `uint4` (128-bit, 32 nibbles/load) + cheaper dequant (fold zero-point) — attacks the plateau on *all* shapes. (Qwen3-VL-32B gap filled: 15.2 @128 / 9.2 @131K, 256K KV 273174, vision PASS.)
...
| Devstral-2-24B | Dense+VL (Mistral3) | 256K / NA | 24 / — | `launch.sh devstral2` | Working. Devstral-Small-2-24B (Mistral3 + Pixtral vision, **image-only**). **AWQ int4 is the dense-VL 256K win**: int4 weights (~½ FP8) free enough VRAM for a **507K-tok KV pool** → full 262144 *with* the BF16 vision tower resident, where FP8 caps ~180K (weight-size bound; raising mem-fraction doesn't help — switching to AWQ does). basic+vision+tool-call all PASS; **241K-tok needle PASS** (coherent decode, not just KV alloc). Decode 24 (2K) / 20.8 (32K) / 18.2 (65K) tok/s, conc=1. FP8 = NA under 256K-only mandate. No draft. Ships `mattbucci/Devstral-Small-2-24B-AWQ`. |
...
| Qwen3-VL-32B | Dense VL | 256K / NA | 15.2 / — | `launch.sh qwen3vl-32b --context-length 262144` | AWQ reaches **full 256K** (KV pool 273174 @mem0.85) — measured 2026-05-31: **15.2 @128 / 9.2 @131K** conc=1 (64-layer dense, dispatch-bound — same ~1 ms/layer as the 27B hybrids; see sweep footnote). **basic+VISION PASS** (content-checked: "red circle … on a white background"). FP8 caps ~159K (<256K) so FP8 = **NA** under the 256K mandate. VL spec broken (#17935). Preset defaults to CTX 32768 — pass `--context-length 262144` for 256K. |
...
All values tok/s single-user, conc=1.  The Qwen3.5-27B / Qwen3.6-27B AWQ rows were **re-measured 2026-05-31** from the clean conc=1 sweep JSONLs (`sglang_0531_1_*`) + live spot-checks — the two DeltaNet-27B hybrids share one curve (~14.4 short → 13.0 @32K → 11.8 @65K → 9.7 @131K → ~7.2 @256K).  **This corrects the earlier 24–26 short-ctx figures, which the current v0.5.12 + patch-041 GEMV stack does not reproduce** (current dense/hybrid AWQ M=1 decode is ~14.4, confirmed by JSONL data + fresh runs; cuda-graph makes no difference — verified `Decode batch … cuda graph: True` still decodes ~13).  ⚠ **Open: decode is GPU-compute-bound on a slow GEMV kernel.** conc=1 decode scales with layer count (Devstral-2-24B 40 layers = 24 tok/s; all 64-layer models — hybrid Qwen3.5/3.6-27B *and* dense Qwen3-VL-32B — = ~14–15, ~1 ms/layer regardless of arch).  Profiling (qwen36-27b, [receipt](benchmarks/profiling/qwen36-27b-decode-profile-2026-05-31.json)) shows **85.8% GPU utilization** — not dispatch-bound, which is why cuda-graph gives no speedup — with **78% of decode time in the patch-041 `awq_gemv_bf16_kernel`, running ~5× under the ~10.5 ms/step weight-read roofline**.  An April-24 sweep hit 24.1 (41.5 ms) on the 64-layer model (predates patch-041, likely better-tuned rocBLAS GEMM).  The lever is optimizing that HIP GEMV kernel — see Open work #2.  †262K from the Qwen3.6-27B 245760-tok point (7.2 tok/s, TTFT 31 s); the family is measured-identical so it applies to both rows.  Both 35B-A3B MoE models hit the 256K target with similar characteristics; Qwen3.6 edges out Qwen3.5 at 256K (13.3 vs 12.4).
