...
2. **Re-sweep the dense/hybrid AWQ decode tables** — the Qwen3.5-27B / Qwen3.6-27B / Qwen3-VL-32B decode numbers in the tables below predate the patch-006 GEMV batched-load opt (~1.7× low); only qwen36-27b @128 (=25) is re-measured. (Optional kernel residual: `uint4` 128-bit loads + cheaper packed dequant could push the GEMV past its current ~50–80% roofline. Landed-fix detail: patches/README.md patch 006.)
