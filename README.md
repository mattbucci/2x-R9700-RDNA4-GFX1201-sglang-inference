...
4. **✅ Nemotron-3-Nano-Omni-30B-A3B-Reasoning-FP8 SERVES on RDNA4 (2026-05-31, user-requested add)** — NVIDIA Mamba2-Transformer hybrid MoE, ModelOpt per-tensor FP8, full AVLM (text+image+video+audio), thinking. Unblocked by **patches 043** (`is_hip` Triton causal_conv1d for `MambaMixer2` — the first Mamba2-hybrid to run on this box) **+ 044** (`modelopt_fp8` ROCm quant-allowlist) + `pip install librosa` (Parakeet sound encoder). Boots + generates coherently at ≤~64–128K (`--quantization modelopt_fp8 --attention-backend torch_native --reasoning-parser nemotron_3 --kv-cache-dtype fp8_e4m3 --trust-remote-code`, TP2). KV pool 3.49M tokens (Mamba state-based). **Serve no-spec** (no MTP/draft; NGRAM CUDA-only). ⚠ **256K still blocked** — see Known Issues (Triton-AMD SSD chunk-scan compiler bug). Remaining: codify a `nemotron-omni` launch.sh preset, find the exact serveable context ceiling, deep-test multimodal. Task #8.
...
- **Nemotron-3-Nano-Omni 256K blocked — Triton-AMD SSD chunk-scan compiler bug.** Serves + coherent at ≤~64–128K (patches 043/044 + librosa), but the 256K bench (131K-token prefill) crashes the scheduler: `_chunk_scan_fwd_kernel` (`mamba/ops/ssd_chunk_scan.py`) fails in `make_ttgir` → `TritonAMDGPUCanonicalizePointers` assertion `fatPtrs.at(then)==fatPtrs.at(else)` ("expected then/else fat ptr canNarrow equal") → `PassManager::run failed`. Triggered only by large-context chunk shapes (divergent fat-pointers in an `scf.if`); the conv1d path (patch 043) runs clean and memory is NOT the limiter (KV pool 3.49M tok). Candidate fixes: disable the AMD canonicalize-pointers pass for this kernel, force a smaller SSD chunk size, or a Triton-AMD compiler upgrade. Find the exact pass/fail context boundary (8K passes, 131K fails). Task #8 follow-up.
