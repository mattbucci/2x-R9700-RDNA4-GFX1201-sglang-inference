...
2. **Qwen3.6-35B v2 calibration ship blockers (2026-04-26, two separate issues).** Recalibrated with corrected `re:.*shared_expert\..*` (singular+dot) — 16h, exit=0, 19GB v2 CT + 19GB v2 native AWQ saved with proper 261-entry ignore list. **(a) llmcompressor saved v2 with `architectures=Qwen3_5MoeForCausalLM` (text-only) instead of v1's `Qwen3_5MoeForConditionalGeneration` (multimodal wrapper)** — same safetensors keys, but the SGLang loader's `language_model.` strip is calibrated for the wrapper class, so the post-strip lookup misses on text-only registration. Fixed by patching `architectures` + `model_type` in v2 config and copying `text_config` + `vision_config` + `{vision,image,video}_token_id` from v1. After patch, v2 boots cleanly on TP=2. **(b) v2 native AWQ then crashes on first inference with HSAIL 0x1016** — same exception class as #18 (Coder-Next, Gemma4-31B long-decode). Hypothesis: v2's BF16 shared_expert (different code path than v1's quantized) trips the same RDNA4-Triton miscompile in BF16 GEMM at the shared_expert shape. v1 stays in production until either #18 is fixed OR v2 is recalibrated keeping shared_expert quantized. *Earlier note that this was a "loader patch" issue was wrong direction — fixing the config is enough to load.*
...
7. **REAM + REAP variant coverage for every MoE model.**  REAM and REAP are **different** expert-pruning strategies — not aliases.  Coder-Next-REAM 60B proves the REAM path works on our stack (25 tok/s @ 131K).  Cerebras's `Qwen3-Coder-REAP-25B-A3B` is a REAP prune of Coder-30B (different algorithm).  Goal: produce + bench + validate both method families per MoE model wherever public weights exist, and self-prune otherwise.  **In flight (2026-04-26):** building REAM-pruned Qwen3.6-35B-A3B (256→192 experts) ourselves via Samsung SAIL `merge.py` (CPU, c4+math+code calibration, ETA ~24-28h on R9700). Output: `~/AI/models/Qwen3.6-35B-A3B-REAM-BF16` then quant pipeline → estimated ~27B AWQ that should fit 256K with comfortable VRAM headroom.  For the remaining MoE models (Gemma4-26B, Qwen3.5-35B-A3B), search HF for both method names separately.  Bench both REAM and REAP independently — don't label one as the other.
