...
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS incl. fused-expert; MoE perf root-caused (native fp8 WMMA confirmed; int4 wins M=1 decode, fp8 wins M≥8).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet (`in_proj_*`,`conv1d`)/all gating (`mlp.gate`, `*_gate`, `*.router.*`) — they stay BF16.
   ...
   **Production call:** for the **single-user pure-decode** mandate target, **AWQ-int4 stays the MoE path** (~1.5× faster at M=1 — fundamental, no config fixes it). **FP8 MoE is the better choice for any batched/compute-bound regime** — prefill-heavy, multi-user, or speculative/MTP verify (M≥8). Dense FP8 (Devstral, par with AWQ at 256K) is unaffected. (Microbench used per-tensor fp8 as a tiling proxy; per-channel scale granularity is negligible at the kernel level.)
   ...
   **Two perf levers tested & ruled out (2026-05-27), single-user decode coder-30B/qwen36-35B FP8 baseline ≈ 17.7 tok/s @8K:**
   ...
   - **MTP speculative decode (NEXTN) — runs but no boost.** qwen36-35B ships a 1-layer MTP head (`mtp_num_hidden_layers=1`); SGLang maps `Qwen3_5MoeForConditionalGeneration`→`Qwen3_5ForCausalLMMTP` draft. It loads clean (compressed-tensors, 0 unloaded params) and stays coherent, but **accept length ≈ 1.0–1.10, accept rate ≈ 0** at both `--speculative-num-steps` 1 and 3 → effectively no accepted draft tokens, so decode does *not* rise into the M≥8 fp8-favorable regime (17.7 → ~15–17, slightly worse from draft overhead). Draft loads fine, so the ~0 acceptance is either the **FP8-quantized MTP head** being too degraded (a 1-layer next-token predictor is far more quant-sensitive than the backbone) or **immature hybrid-DeltaNet + spec-decode on the RDNA4/Triton linear-attn backend** (SGLang auto-disables radix cache for this combo on ROCm — a sign of partial support). Disambiguation follow-up: re-cast keeping `mtp.*` in BF16 and retest; if acceptance is still ~0, it's the DeltaNet+spec path.
