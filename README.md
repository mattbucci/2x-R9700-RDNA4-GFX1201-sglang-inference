...
### Next steps (prioritized — 2026-06-14, single-user 256K)

1. **[IN PROGRESS] EAGLE3 spec-decode on the dense DeltaNet 27B (Qwen3.6-27B / Qwen3.5-27B).** These two primary 256K models are currently spec-decode **blocked** (Open work #5) — but only the 3.3 GB **DFlash** (OOMs DeltaNet) and an in-ckpt **MTP recast** were ever tried. A small **EAGLE3** draft (`LlamaForCausalLMEagle3`, 1.25 GB, full 262144 ctx, geometry-matched, aux layers [3,31,59]) is on disk at `models/Qwen3.6-27B-EAGLE3-specdrift` and was never benched. EAGLE3 is ~⅓ the size of DFlash so the DeltaNet OOM concern doesn't apply. **Bench it:** boot `qwen36-27b` + this draft with `--speculative-algorithm EAGLE3 --speculative-attention-mode decode`, measure **server-log `gen throughput`** (not client TPOT — it under-measures bursty spec ~2×) vs the ~25 short / 10.2 @256K no-spec baseline, confirm coherence (basic+thinking). If it accepts, flip 27B blocked→working, update `specdecode.json`/charts, push to 3090 (their dense 27B is blocked too).
2. **Re-measure the two working spec bars under cuda-graph + server-log method** (Open work #5 tail). Coder-30B EAGLE3 (97/86) and Qwen3.6-35B DFlash (80/45) predate the 2026-06-01 graph-ON sweep AND were taken with the client tool that under-measures spec ~2×. Re-bench → correct `specdecode.json` + `fp8-comparison.json` + both charts.
3. **North-Mini long-ctx agentic scheduler hang** (Known Issues) — bound agentic context or root-cause the hybrid-SWA `cohere2_moe` long-ctx stall; it's decode/short-ctx-agentic-validated but not long-ctx-agentic-robust.
4. **TurboQuant KV de-risk** (Open work #4, lower priority) — build the `domvox` llama.cpp HIP TurboQuant fork on gfx1201 and bench decode-neutrality as a *comparison* before committing to the SGLang fused-triton-decode kernel.
5. **Upstream the 10 PR candidates** (Open work #6) — blocked on a `GH_TOKEN` with fork scope.
...
3. **North-Mini-Code-1.0 follow-ups** — the FP8 ship is live (`launch.sh north-mini`; in the model + sweep tables). ✅ (a) **`force_rope`→`mlp_layer_types` fix applied 2026-06-14** (live in `/data/vG` + captured in the candidate): the dense L0 was reading the popped `first_k_dense_replace` (always 0 post `__post_init__`) so `force_rope` was always False → L0 (a full-attention dense-prefix layer) ran **NoPE** instead of RoPE. Now reads `config.mlp_layer_types[i]=="dense"` at *both* the MoE-selection and `force_rope` sites (matches the 3090's patch-042 fix). ✅ (b) **`cohere_command4` reasoning parser grafted 2026-06-14** (patch 053, from upstream main): routes `<|START_THINKING|>…<|END_THINKING|>` to `reasoning_content` and strips the `<|START_TEXT|>…<|END_TEXT|>` response wrapper from `content` (the delimiters are `special=False`, so `skip_special_tokens` couldn't). Validated non-stream + stream on North-Mini-Code-1.0-fp8: `reasoning_content` populated, `content` delimiter-free. ✅ (b2) **`cohere_command4` tool-call parser grafted 2026-06-14** (patch 054, from upstream main): parses `<|START_ACTION|>[…json…]<|END_ACTION|>` into structured `tool_calls` (normalizes Cohere's `tool_name`→`name`, drops `tool_call_id`). Preset now passes `--tool-call-parser cohere_command4`; composes with the reasoning parser. Validated non-stream + stream on North-Mini-Code-1.0-fp8: a tools request returns `finish_reason: tool_calls` with `get_weather({"city":"Paris"})` structured, `reasoning_content` populated, `content` empty/clean. **North-Mini is now fully agentic-ready** (thinking + structured tool calls + clean content) — confirmed by a 2-instance opencode `--no-venv` smoke (served as `sweep`): preflight OK, **2/2 instances produced non-empty patches** (real git diffs, `rc=0`), so the grafted parsers actually drive tool-based edits end-to-end. ✅ (c) **first scored agentic bench 2026-06-14: 6/9 django resolved (66.7%)** via opencode + `score_local` (no docker) — rollout 14/15 non-empty patches, 7/9 applied; validates the parsers drive real SWE-bench resolution (see the FP8 SWE-bench table). ⚠ this was the django-9 slice (first-15 Lite, astropy excluded as no-docker-unscoreable), NOT the standard 15-instance buildable subset — so the apples-to-apples full-subset run vs the Coder-30B/REAP fleet remains. (Scorer needed a `swebench` venv — now at `/tmp/swebench-scorer-venv`.) Candidates `patches/051-cohere2moe-model-graft.patch.CANDIDATE`, `patches/053-cohere2moe-reasoning-parser.patch.CANDIDATE`, `patches/054-cohere2moe-toolcall-parser.patch.CANDIDATE`.
4. **[PLANNED — reassessed 2026-06-14, downgraded] TurboQuant KV-cache quantization (ICLR 2026, [arXiv:2504.19874](https://arxiv.org/abs/2504.19874)).** Data-oblivious online vector quant of the K/V cache (random rotation → concentrated per-coordinate Beta → optimal scalar quantizers): typ. 3-bit keys / 2-bit values vs our 8-bit `fp8_e4m3` KV → **~2.3–5× smaller KV**. **Upstream-state audit (don't repeat the optimistic framing):**
   - **SGLang path is NOT a patch-port.** The cited issue [#21618](https://github.com/sgl-project/sglang/issues/21618) is **closed/inactive**; its WIP PR (#21617/#21419) is **CUDA/FlashInfer-only** and has a **catastrophic decode bottleneck — it dequantizes the entire KV pool every attention call → ~0.03 tok/s on H200** (exactly the per-step-dequant risk this gate warned about). The decode-neutral fix — fused Triton decode/extend kernels that read packed 4-bit KV in-attention (proposed [#23134](https://github.com/sgl-project/sglang/issues/23134), claims 93–105% of bf16) — is **only proposed, not built**. We have **no FlashInfer on gfx1201** (triton-only), so adopting it here = *implementing the unbuilt fused-triton-decode path in our triton attention*, a multi-week kernel-dev effort, not a patch.
   - **The approach IS RDNA-viable + decode-neutral — proven in llama.cpp, not SGLang.** The community HIP fork `domvox/llama.cpp-turboquant-hip` works on gfx1100/1101 (RDNA3): Qwen3.5-27B turbo3 = **PPL ±0%, decode −2% (30.2→29.5 t/s), 5.1× KV compression** (80K ctx OOM@26GB → 20GB). So the algorithm clears our decode-neutral gate on RDNA — but llama.cpp is a **comparison engine only** (mandate = serve on SGLang); it doesn't give us a shippable path.
   - **Value is narrower than first framed.** The dense/VL models this was meant to "unlock" — Qwen3-VL-32B, Devstral-2-24B — **already reach full 256K single-user via AWQ** (int4 weights are ½ FP8 → KV fits; see their model-table rows). So TurboQuant does NOT gate single-user 256K. Its real near-term value is the **FP8 *agentic* lane**: FP8 is the agentic path for dense *thinking* models (int4 degrades agentic — see Known Issues), and FP8 dense thinking (Qwen3.5/3.6-27B) caps <256K — TurboQuant KV would let those reach 256K *in FP8*. Plus general headroom for batched/spec regimes.
   - **Revised plan (lower priority):** (1) **de-risk cheaply** — build the `domvox` llama.cpp HIP TurboQuant fork on **gfx1201** and bench it as a comparison (confirm RDNA4 runs the kernels decode-neutral, since only RDNA3 is tested); (2) only if that holds AND FP8-256K-agentic is prioritized, undertake the SGLang triton fused-decode kernel implementation. Gate unchanged: single-user M=1 decode tok/s must NOT regress. Refs: [SGLang #21618](https://github.com/sgl-project/sglang/issues/21618) / [#23134](https://github.com/sgl-project/sglang/issues/23134), [llama.cpp HIP fork #21526](https://github.com/ggml-org/llama.cpp/discussions/21526).
5. **Expand spec-decode coverage — land a working draft on as many models as we can.** Status tracked in [`benchmarks/specdecode.json`](benchmarks/specdecode.json) → fleet chart [`benchmarks/specdecode_fleet.png`](benchmarks/specdecode_fleet.png). **7 already working** — the whole A3B-MoE coder/REAM/REAP family rides EAGLE3/DFlash transfer drafts at 256K (Qwen3.6-REAM-A3B 133, Coder-30B-REAM 101, Coder-30B 97, REAP-25B 91, Coder-30B-REAP 90, Qwen3.6-35B-A3B 80; Coder-Next-REAM 58 @64K). Remaining targets: **(untested — a draft path exists, just bench)** Qwen3-Coder-Next-80B (Aurora-Spec EAGLE3); **(blocked — needs work)** Qwen3.5-28B-A3B-REAP — **tested 2026-06-14, no useful speedup** (downloaded `z-lab/Qwen3.5-35B-A3B-DFlash` parent draft, served via the qwen35-moe preset with `--speculative-attention-mode decode`: loads, coherent, 4/4 caps incl. vision+video, accepts tokens — accept len 2–8 workload-dependent). **Authoritative server-log sustained gen-throughput ≈52 tok/s at short ctx vs the ~60 no-spec baseline → net-negative-to-neutral**: the DFlash block-diffusion draft+verify overhead roughly cancels the acceptance benefit, and decode still collapses at deep context (≤~2 tok/s by ~240K — same DeltaNet+DFlash deep-context wall as Coder-Next-REAM; at 256K with chunked-prefill 8192 a verify child OOM-SIGQUITs, cleared at cp2048 but still no win). Corroborates the 3090's "DFlash moot on the Qwen DeltaNet-MoE family." Not a viable spec win — reuse not recommended. ⚠ **Measurement note:** the streaming-client TPOT path (`measure_decode_curve.py`) **under-measures bursty spec decode ~2×** (SSE buffering of accepted blocks) — it read 25.5@128 here where the server-log gen-throughput is ~52; **read the server-log `gen throughput` for spec, not client TPOT.** DeltaNet-dense 27B pair (Qwen3.5/3.6-27B: DFlash OOMs DeltaNet even @16K → path is AWQ-int4 + BF16-MTP recast), gemma-4-31B/26B (no DFLASH hook; FROZEN_KV_MTP needs transformers 5.8 + an SGLang fix; base EAGLE3 accept only 1.34 → needs an it-matched draft), Qwen3-VL-32B + VL-REAP-26B (SGLang VL spec bug [#17935](https://github.com/sgl-project/sglang/issues/17935), missing `get_embed_and_head`), Devstral-24B (no published draft → SpecForge-train a `MistralForCausalLMEagle`), Nemotron-Omni (no bundled draft, Nano excluded from MTP). Also **re-measure the existing AWQ + FP8 spec bars under cuda-graph** (current spec numbers predate the 2026-06-01 graph-ON sweep). As each lands, flip its `specdecode.json` status to `working` + regenerate both charts (`scripts/bench/generate_charts.py`).
6. **Upstream the 10 PR candidates** (031 033 034 037 043 044 046 047 048 049) — blocked on a `GH_TOKEN` with fork scope (403 on `gh repo fork`). At the next version bump: drop 012+035 (upstreamed in main), rebase 040 onto main's compact `[ARGS]` parser, and decide 032 (skinny-GEMM, currently unwired) wire-or-archive. Full patch map + upstream-lifecycle scorecard in [PATCHES.md](PATCHES.md).
...
- **North-Mini (`cohere2_moe`) scheduler hangs under sustained long-context agentic load (2026-06-14).** During the full-15 buildable SWE-bench rollout, the server completed 12/15 instances then **hung on a large-repo prompt** (pydata xarray): the scheduler heartbeat stopped (`last_heartbeat 17:14:38`), no forward progress for ~40 min, and the 1800s scheduler watchdog fired → `SIGQUIT`. The opencode per-instance 600s timeout did NOT recover it (the hang is server-side, so the client just waits on a dead server). Short/mid-context agentic works (it resolved 6/9 django + produced patches through seaborn/flask/requests/xarray). Likely a long-context decode/prefill hang on the hybrid-SWA `cohere2_moe` path under big agentic prompts — same *class* as the qwen36-27b emergent-hang above (real agentic traffic + long ctx), distinct arch. **Aftermath:** the SIGQUIT leaked ~30 GB on GPU[1] that `rocm-smi --showpids` shows as un-drainable (no live KFD pid) — needs a drain/reset before the next serve (the "un-drainable VRAM" case from `git log` `d65dcd9`). **Next:** bound agentic context for North-Mini (cap opencode context / chunked-prefill) or root-cause the long-ctx scheduler stall; for now treat North-Mini as decode/short-ctx-agentic-validated, not long-ctx-agentic-robust.
...
- **int4 agentic 0/6 is an int4 × triton-attention interaction at long KV, not an int4 ceiling (3090 `da1a58e`)** — int4+FlashInfer (3090) is clean (qwen36-moe 59%) while int4+triton (R9700) = 0/6 and fp8+triton = 4/6; patch-011's triton BF16 precision loss compounds over KV and tips int4's noisy tool-format logits into malformed emissions only at long context. **Open R9700 isolator:** re-run the int4 smoke with `--attention-backend torch_native` *inside* the multi-turn harness — 0→nonzero ⇒ the fix is the triton-attn rekernel, not an FP8 fallback. Tool: `scripts/eval/context_reliability_curve.py`.
