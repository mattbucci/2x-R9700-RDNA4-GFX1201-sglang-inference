...
Next:
0. **FP8 lane (2026-05-27, R9700-owned per team split) — dense PROVEN, MoE WORKS (perf follow-up open).** gfx1201 native FP8 accel → FP8 W8A8 for the int4 ships. FP8_DYNAMIC = per-output-channel symmetric FP8 weights (compressed-tensors `float-quantized`) + dynamic per-token FP8 activations; ignores lm_head/vision/DeltaNet/MoE-router (stay BF16).
   ...
   **Findings (corrects the earlier optimistic context-math):** (1) **Dense ≤24B (Devstral) is the clean FP8 win** — par speed, fits 256K. (2) **FP8 weights are 2× int4, so max context DROPS for larger/hybrid models near the 32GB limit**: 32B-dense → ~159K, 31B-dense(Gemma SWA) → ~51K, 27B-DeltaNet (mamba-state cache eats VRAM) → ~34K. AWQ-int4 reaches 262K on all of these. So FP8 *costs* context above ~24B — NOT "all dense reach 256K". (3) **A3B-MoE keeps a huge KV budget** (coder-30B 522K, qwen36-35B 1.75M tokens) — only 3B active, weights/card modest. (4) Vision + thinking + DeltaNet all survive FP8 (towers/SSM-projections kept BF16). (5) **FP8 MoE serves but ~2× slower than AWQ** (perf TODO below).
   ...
   ⚠ **Fused-expert FP8 GAP — Qwen3.6-35B-A3B AND gemma-4-26B (both confirmed fused):** Both store experts as 3D fused `experts.gate_up_proj`/`experts.down_proj` (gemma-26B: `[128,1408,2816]`/`[128,2816,704]`). Qwen3_5Moe path: SGLang's `models/qwen3_5.py` custom loader takes these FUSED and `chunk(2,dim=-2)`s them itself → maps to `experts.w13_weight`/`w2_weight`. The manual caster's unfuse-to-per-expert was WRONG (→ incoherent output). FIX PATH: keep experts fused, emit `experts.gate_up_proj`/`down_proj` as FP8 3D + per-expert-per-channel scales; open question is the scale tensor name the loader's `name.replace(experts.gate_up_proj→experts.w13_weight)` expects (likely `experts.gate_up_proj_scale`→`experts.w13_weight_scale`). coder-30B (native per-expert) is the working MoE FP8 reference.

   ⚠ **TODO — FP8 MoE performance (dig in later).** Coder-30B FP8 MoE serves correctly but 14.7 tok/s vs AWQ-int4 ~30 — the opposite of the lane's premise (native gfx1201 FP8 should *beat* int4). Boot logs "Using default MoE kernel config … Config file not found" → no tuned Triton MoE config for gfx1201 expert shapes (generic tiles). Next: (a) **inspect the generated Triton MoE kernels in the cache** (`TRITON_KERNEL_DUMP=1`, then read `~/.triton/cache/<hash>/fused_moe_kernel.{ttgir,amdgcn}`) to see the tile sizes the default config picked and, in the amdgcn, whether it emits native gfx1201 FP8 MFMA/`v_dot` or upcasts FP8→BF16 before the matmul — per the CLAUDE.md Step-4 kernel-dump method; (b) from that, generate a tuned `E=…,device_name=…,dtype=fp8.json` config for our expert geometries; (c) if the kernel upcasts (no native FP8 dot), no FP8 speedup is achievable on gfx1201 and AWQ-int4 stays the MoE production path. Dense FP8 (Devstral, par with AWQ) is unaffected.
