...
3. **256K single-user context sweeps (partial data gathered)** — Numbers so far (TPOT-based, FP8 KV, --disable-cuda-graph):
   - **Qwen3.5-27B AWQ**: 26 tok/s @ 128-2K → 21.4 tok/s @ 16K → 18 tok/s @ 32K → 14.1 tok/s @ 65K.  Killed at 131K on first run (old 600s subprocess timeout); will rerun now that the timeout scales with context.
   - **Qwen3.5-35B MoE GPTQ (full 256K sweep done)**: 14-16 tok/s @ 128-8K → 14.4 tok/s @ 16K → 16.7 tok/s @ 32K → 14.7 tok/s @ 65K → 15.3 tok/s @ 131K → **12.4 tok/s @ 262K, TTFT 58s**.  MoE+DeltaNet holds up impressively: only 14% TPOT degradation from 128 → 256K context.  Contrast with dense-equivalent Qwen3.5-27B which fell to 14 tok/s by 65K alone.
   - Queued: Devstral-24B, Coder-Next 80B, Coder-Next REAM 60B, and (post-calibration) Qwen3.6-35B.
   - Caveat: the 262K-context launch presets (MEM=0.85, MAX_RUNNING=8, 2-3M-token KV pool) give ~30% slower short-context TPOT than the old 32K-optimized presets.  That's the trade for 256K headroom; override with `--context-length 32768 --max-running 32` for multi-user throughput.
4. **Gemma4 reasoning parser (patch 014)** — Landed (`Gemma4Detector` for `<|channel>` / `<channel|>`). Shipped to 3090 team. Next: verify streaming behavior with `--reasoning-parser gemma4` in a long-context agent workload.
