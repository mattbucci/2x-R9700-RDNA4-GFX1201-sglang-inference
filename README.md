...
3. **North-Mini-Code-1.0 follow-ups** — the FP8 ship is live (`launch.sh north-mini`; in the model + sweep tables). Remaining: (a) apply the 3090's `force_rope`→`mlp_layer_types` fix — the dense L0 still reads the popped `first_k_dense_replace` and silently runs **NoPE** (positional-encoding quality bug, not a crash); (b) graft the `cohere_command4` tool/reasoning parser; (c) agentic-bench vs the Coder-30B / REAP fleet. Candidate `patches/051-cohere2moe-model-graft.patch.CANDIDATE`.
