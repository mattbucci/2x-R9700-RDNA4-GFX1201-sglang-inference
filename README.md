...
2. **DeltaNet/mamba-hybrid decode regressed ~1.7× in v0.5.12 — recover the cuda-graph amortization.** The DeltaNet-27B AWQ models (qwen35, qwen36-27b) decode ~14.4 tok/s conc=1 short-ctx on v0.5.12 (TPOT ~69 ms eager; measured 2026-05-31, three sources agree); an April-24 sweep on the older SGLang got **24.1 tok/s (TPOT 41.5 ms)** with cuda-graph (the tell-tale ~4.8 s capture time landed in TTFT). The deficit is **not** the AWQ Linear — dense **Devstral-2-24B AWQ decodes 24 tok/s (TPOT 41.7 ms)** on this same stack, so the GEMV path is healthy. It's the 64-layer hybrid's per-layer decode **dispatch**: in v0.5.12, cuda-graph for these mamba/DeltaNet hybrids is **ineffective** (`Decode batch … cuda graph: True` still ~13 tok/s, *slower* than eager 14.4 — the graph capture doesn't amortize the dispatch), whereas the April stack's graph took it down to dense-equivalent 41.5 ms. Not a `--quantization` flag (on ROCm `awq` == auto-detect; `is_awq_marlin_compatible` gates on `_is_cuda=False`). Fix = make v0.5.12 cuda-graph actually capture/amortize the DeltaNet+mamba decode (same kernel family as the spec-decode mamba block, README known issues). Single-user lever across every DeltaNet hybrid. Also fill the **Qwen3-VL-32B** no-spec tok/s gap (`—` in the Agent table) at 256K — it's dense (no DeltaNet) so expect ~Devstral-class decode, not the hybrid deficit.
