...
1. **Qwen3.6-35B-A3B (2026-04-18 release)** — Same MoE+DeltaNet architecture as Qwen3.5-35B (patch 009 covers it), but thinking-by-default and native multimodal.  Community GPTQ `palmfuture/Qwen3.6-35B-A3B-GPTQ-Int4` downloaded locally; `scripts/quantize/flatten_qwen36_config.py` flattens the nested config for SGLang, then launch via `qwen36-moe` preset.  3090 team got 14 tok/s @ 250K; we expect similar.
