...
- **Vision support** — Devstral-24B, Qwen3.5-27B, Gemma 4 31B: vision WORKING. Gemma 4 26B MoE: text works via mm model (expert loading FIXED — added `FusedMoE.make_expert_params_mapping` to mm `load_weights`). Vision with actual images crashes in vision encoder SDPA on RDNA4 (HSA exception). Vision encoder SDPA fallback from triton_attn is in place but still crashes with real image data. Without `--enable-multimodal`, text works and vision responses say "Blue" (MoE routing misalignment from text-only calibration).
