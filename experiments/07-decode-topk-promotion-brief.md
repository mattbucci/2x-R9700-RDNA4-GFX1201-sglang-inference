...
| **Depends on** | User decision (A/B/C) — this is the blocking item; the README queue marks it a user call.; Stability of uncommitted in-flight patches 083-089 (working tree is current state): regen must be cut against that tree and appended after 089; 090's server_args hunk must coexist with 086 (which also edits server_args.py).; A ~256K needle context fixture: /tmp/spec256k-context.txt does not currently exist and must be (re)generated, or CTX re-pointed at a repo-root 245760 fixture, before steps 7-8.; No concurrent calibration/pruning on r9700-box during the boot-heavy re-gate steps 5-9. |
...
- In-flight interaction: uncommitted patches 083-089 include 086 (086-rdna4-amd-num-kv-splits-64.patch — VERIFIED to edit python/sglang/srt/server_args.py) and 087 (087-rdna4-flash-decode-bf16-pv.patch, touches triton_ops/decode_attention.py — a file 069 does not touch; 069 touches triton_backend.py). Regeneration must happen on the current fully-patched working tree and be appended AFTER 089 (as 090-093); the new 090 server_args hunk must be re-anchored to coexist cleanly with live 086, which lands in the same file after 089.
...
3. Regenerate the 4-patch unit against the current fully-patched tree: in a scratch worktree copy of /data/sgl-v0515, port 067, 068, 069, 070 by hand onto the v0.5.15 annotated-ServerArgs style (dataclass fields become A[...] annotated fields; forward_decode/init hunks re-anchor in triton_backend.py; 070's guard re-anchors in models/utils.py). Emit patches/090-decode-window-v0515.patch, 091-partial-verify-window-v0515.patch, 092-decode-topk-sparse-v0515.patch, 093-decode-window-qwen3moe-fix-v0515.patch (090-093 verified free). Fix 092's help-text doc reference to benchmarks/FINDINGS.md. Explicitly confirm 090's server_args.py anchor sits cleanly relative to live 086 (086-rdna4-amd-num-kv-splits-64, which also edits server_args.py) — both land in the same file after 089, so verify no overlapping hunk context.
4. Run the rebase gate per the version-rebase method: python -m py_compile on all touched files, then the eager-import boot-chain smoke (python -c 'import sglang.srt.server_args; import sglang.srt.layers.attention.triton_backend'), then verify git apply --check of 090-093 FAILS on the already-patched tree and succeeds on a pristine-plus-001-089 replay (equivalence-gate convention).
5. Apply 090-093 to /data/sgl-v0515. Boot smoke without the flag first: ./scripts/launch.sh qwen3vl-32b on a spare port; confirm no behavior change and no graph warning (flag default -1 = off). This is the no-op safety check.
...
- If B executed: 090-093 apply cleanly on pristine+001-089 replay, py_compile + eager-import smoke pass, and git apply --check fails on the patched tree (already-applied proof); 090's server_args hunk coexists with 086 without overlap.
- No-flag boot of qwen3vl-32b after applying 090-093 shows no graph warning and unchanged serving (no-op-when-off proof).
...
- Regeneration forces semantic changes to in-flight 083-089 (not just offsets/re-anchoring) — stop and escalate to the user rather than destabilize the Laguna FP8 lane.
...
- If B: patches/090-decode-window-v0515.patch, 091-partial-verify-window-v0515.patch, 092-decode-topk-sparse-v0515.patch, 093-decode-window-qwen3moe-fix-v0515.patch (old 067-070 renamed .SUPERSEDED).
...
- 090's server_args hunk lands in the same file as live 086 (rdna4-num-kv-splits) after 089 — an overlapping anchor could re-order or fail silently; step 3 explicitly checks this.
