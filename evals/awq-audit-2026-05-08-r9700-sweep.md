...
| `qwen36-27b` | ✅ **3/3 PASS** post DECODE_STEPS=8 + validator knob | ✓ paris finish=stop | ✓ reasoning_seen answer_ok TRUNCATED (512 tok, finish=length) — passes via the truncated-but-answer-found gate | ✓ red+circle+round → 'red circle on a white background, resembling the flag of japan' | **2026-05-08 ROOT CAUSE FOUND + RESOLVED.** Diff vs qwen36-moe (3/3 PASS sister): qwen36-27b ran DECODE_STEPS=32, qwen36-moe ran DECODE_STEPS=8. Manual A/B on R9700 with `--decode-steps 8`: thinking 1024 tokens completes finish=stop, correct answer; before fix server crashed scheduler. Vision now content-aware (recognizes red circle, compares to flag of Japan). Validator default 8192 max_tokens exceeds the 300s probe budget at decode=8. **Validator now ships `--max-tokens-thinking` CLI override** (commit pending) — at 512 tok the thinking probe completes in 121s with reasoning_seen + answer_correct, passing the same gate as a longer chain. Sweep entry now FULL PASS. Fix combo: `DECODE_STEPS=8` (preset) + `--max-tokens-thinking 512` (validator). |
...
| `coder-30b` | ✅ **1/1 PASS** post fix | ✓ paris finish=stop in 0.7s | (skip — non-thinking) | (skip — non-vision) | **2026-05-08 ROOT-CAUSED + FIXED.** Initial sweep failure at MEM=0.85/CTX=4096 was misdiagnosed as OOM — actual fault was the launch.sh case missing `DTYPE=bfloat16` and `QUANT=moe_wna16` overrides, falling through to global defaults `dtype='float16'` + `quantization='awq'`. The plain-AWQ path routes 128 experts × 48 layers × 3 projs = 18,432 individual per-Linear AWQ kernels per forward pass on RDNA4 — too inefficient + triggers HSAIL 0x1016. Sister Qwen3MoE presets (qwen35-moe, coder-reap-25b, qwen36-moe) all run on `moe_wna16` + bf16 cleanly via the fused MoE Triton kernel. Adding both overrides to coder-30b case → `[PASS] basic finish=stop answer='paris' --- 1/1 passed in 0.7s`. Memory budget at MEM=0.85/CTX=4096 fits cleanly with the right kernel path. |
...
Out of 8 attempted presets (+1 pre-sweep + 1 post-sweep extension), after patch 031 + DECODE_STEPS=8 + coder-30b moe_wna16 fix + validator `--max-tokens-thinking` knob land:
- **7 fully PASS** (qwen3vl-32b 3/3, devstral 1/1, qwen35-moe 2/2, coder-reap-25b 1/1, qwen36-moe 3/3, coder-30b 1/1, qwen36-27b 3/3)
- **1 partial** (gemma4-31b 2/3 — vision is upstream Google limit, model itself can't recognize the red circle even at BF16)
- **1 distinct failure class** still open (down from 3):
...
| ~~OOM ceiling + HSAIL on first inference~~ | ~~coder-30b~~ | RESOLVED 2026-05-08 — coder-30b case in launch.sh was missing `DTYPE=bfloat16` + `QUANT=moe_wna16` overrides; default fp16 + plain-AWQ path routed 18,432 per-Linear quant kernels through the wrong code path. Adding both overrides → 1/1 PASS in 0.7s at default MEM=0.85/CTX=4096. |
