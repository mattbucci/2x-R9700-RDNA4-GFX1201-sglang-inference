...
NE=96  # full REAM expert count; build all so kernel buffer matches live
EX=list(range(NE))
x=torch.randn(1,2048,device='cuda',dtype=torch.float16)
TK=8;sel=torch.tensor([77,36,1,41,74,87,14,46])%NE
tw=torch.softmax(torch.rand(1,TK),1).cuda();ti=sel[None].cuda()
w13=[];w2=[];s13=[];s2=[];z13=[];z2=[]
for e in EX:
...
refsum=0
for j in range(TK):
 e=sel[j].item();P=f"model.layers.0.mlp.experts.{e}";gg,uu,d=dq(*g(P+".gate_proj")),dq(*g(P+".up_proj")),dq(*g(P+".down_proj"))
 h=torch.nn.functional.silu(x.float()@gg)*(x.float()@uu);refsum=refsum+tw[0,j].item()*(h@d)
...
print("96EXP KERNEL std",y.float().std().item(),"max",y.abs().max().item(),"ratio",y.float().std().item()/refsum.std().item())
