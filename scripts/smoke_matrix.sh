...
for r in $($PY -c 'import json;print(" ".join(json.load(open("/tmp/snap.json"))))'); do
  ...
  Q=$($PY -c "import json;print(json.load(open('$M/config.json')).get('quantization_config',{}).get('quant_method','awq'))")
  ...
  setsid env HF_HUB_OFFLINE=1 ROCR_VISIBLE_DEVICES=0,1 PYTHONPATH=kernels/awq_hip $PY -m sglang.launch_server \
    --model-path "$M" --quantization "$Q" --tp 2 --context-length "$C" --kv-cache-dtype fp8_e4m3 \
  ...
  for i in $(seq 1 50); do curl -sf localhost:$PORT/health>/dev/null && break; pgrep -f sglang.launch_server>/dev/null||break; sleep 10; done
