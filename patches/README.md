...
| 023 | gemma4-moe-mlp-no-quant-config | 14 | **Ported from 3090 2026-05-03.** On Gemma 4 26B `enable_moe_block=True` layers, the dense MLP branch ships as plain BF16 (not AWQ-packed); passing `quant_config` makes the AWQ loader feed BF16 into the qweight slot → NaN/Inf cascade in gate_up_proj → `<pad>` spam in vocab. Fix: gate `quant_config=None` on MoE-block layers. **Pending post-VL-32B test on R9700.** |
| 024 | gemma4-mm-towers-no-quant-config | 16 | **Ported from 3090 2026-05-03.** Gemma 4 vision_tower / embed_vision / audio_tower / embed_audio are kept BF16 by our calibration recipe (per `re:.*vision_tower.*` ignore). Passing `quant_config` allocates `weight_packed` slots that BF16 keys can't fill → vision tower stays at random init → vision hallucinates while passing keyword checks. Fix: pass `quant_config=None` to all four mm tower constructors. **Pending post-VL-32B test on R9700; may explain why our `mattbucci/gemma-4-26B-AWQ` "vision PASS" needs re-verification with content-based prompts (not just round/circle keyword grep).** |
