...
**Verified 2026-06-16 (v0.5.13.post1 rebase):** the full 37-patch series applies clean on a pristine `v0.5.13.post1` clone and reproduces the rebased+validated tree (`/data/sgl-rebase`, env `sglang-triton36-v0513`) **byte-for-byte** (git-diff equivalence gate, empty). Validated by booting two presets TP=2: coder-30b codegen STRONG; gemma4-26B basic+thinking+vision (content-aware probe) + `validate_capabilities` 4/4 (basic/thinking/vision/video). Full receipt: [v0513-rebase-2026-06-16.md](v0513-rebase-2026-06-16.md). ⚠ The **live serving tree is still `/data/vG` (v0.5.12, env `sglang-triton36`)** — the v0.5.13 stack is validated and staged but **not yet promoted to live**; that swap is a separate deliberate step. `components/sglang` remains a stale scratch workspace (see "Tree layout").
