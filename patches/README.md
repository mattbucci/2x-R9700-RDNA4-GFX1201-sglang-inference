...
**37 patches** applied in numeric order on a stock `git checkout v0.5.12`. Source of truth for **what's fixed and how**; main [README.md](../README.md) tracks current state. Cross-collection map (all 4 patch dirs + upstream-lifecycle scorecard): [PATCHES.md](../PATCHES.md).

**Verified 2026-06-10:** the full series applies clean on a pristine v0.5.12 clone and reproduces the live serving tree (`/data/vG`, the editable install every preset imports) **byte-for-byte**. Two live-only edits were folded back during the audit: stray debug print removed from 001, loading-timeout bump captured as 048. ⚠ `components/sglang` is NOT the serving tree — it's a stale rebase workspace (see "Tree layout" below).

**Upstream-main status (audited 2026-06-10 vs `sgl-project/sglang@70c71ba`):** 012 + 035 are now upstream → drop both at the next version bump. 040 is half-upstream (compact `[TOOL_CALLS]name[ARGS]` parsing landed; our marker-omission recovery didn't → rebase, don't drop). Still missing upstream — PR candidates, smallest first: 031/044 (ROCm quant allowlist one-liners), 037 (flashinfer import guard), 033 (gelu assert), 034 (Inf detection), 043 (HIP causal-conv1d), 047 (hybrid v_head_dim duck-type), 049 (conv1d col-load dtype cast), 046 (Triton AMD divergent-pointer fix — also Triton-upstream material), 048 (cold-cache load timeout). **10 candidates** — the index `main:PR` column is the source of truth; 033/037 were missing the marker and 049 was missing from this list before the 2026-06-11 reconciliation ([PATCHES.md](../PATCHES.md#sglang-series--by-upstream-lifecycle-37)).

v0.5.11→v0.5.12 rebase (2026-05-26): 001/003/004/007/011/037 regenerated, old 008/029/038 dropped (upstreamed); the current 008 (sgl-kernel build arch) is an unrelated new patch reusing the freed slot.
...
| 012 | rdna4-sliding-window-decode-fix | 168 | ✓ drop | `torch_native` SWA support for decode/extend; translate full pool → SWA pool; without it, Gemma 4 crashes on any seq > window |
| 034 | sampler-inf-detection | 18 | PR | **Added 2026-05-11.** Adds `isinf(logits)` check parallel to `isnan` in `Sampler._preprocess_logits`. `softmax(+Inf) → NaN` cascades through to `multinomial`/`gather` and faults with `HSAIL 0x1016` async; the existing NaN check missed Inf entirely. Now `--enable-nan-detection` covers both NaN and +/-Inf logits with the same warn+replace+optional-raise behavior. |
...
|-----------|---------|--------|
| SGLang | v0.5.12 | stock + 37 patches (rebased from v0.5.11 — see header) |
