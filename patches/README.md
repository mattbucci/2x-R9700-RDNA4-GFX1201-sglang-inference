...
| 038 | rdna4-wire-hybrid-w4a16-moe-runner | 167 | **Added 2026-05-12.** Wires the wvSplitK INT4 MoE kernel (added by patch 032) into SGLang's `MoeRunner`. Adds `MoeRunnerBackend.HYBRID_W4A16` to the enum (utils.py), an elif dispatch branch in `MoeRunner.__init__` (runner.py) that side-effect-imports a NEW `moe_runner/hybrid_w4a16.py` module which registers a `("none", "hybrid_w4a16")` fused-func via `@register_fused_func`. That fused func builds moe_align metadata + invokes `hybrid_w4a16_moe_apply` from the existing `layers/moe/hybrid_w4a16_moe.py`. Finally, `MoeWNA16Method.create_moe_runner` (moe_wna16.py:365) is changed from hardcoded `MoeRunner(TRITON, ...)` to env-gated selection: prefers HYBRID_W4A16 on ROCm when `SGLANG_MOE_HYBRID_W4A16=true` AND `skinny_gemms_int4_ext` is importable, else falls back to TRITON. Includes a `.value` → `.get()` fix for hybrid_w4a16_moe.py:230-231 (patch 032 used the wrong SGLang env API). **Caveat:** wvSplitK expects scales in skinny `[E, N, K/G]` layout but TritonMoeQuantInfo stores `[E, K/G, N]`; the scale transpose is deferred to a follow-on patch (live-serving validation is gated on that). Surfaced/built 2026-05-12 driving task #28 in /loop dynamic mode. |
