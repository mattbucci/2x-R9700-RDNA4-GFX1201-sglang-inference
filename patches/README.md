...
| 023 | gemma4-moe-mlp-no-quant-config | 36 | **Detection upgrade ported from 3090 2026-05-09 (commit `37daf95`, mirrors 3090 commit `e5e2391`).** Original (May-3) hardcoded `quant_config=None if _enable_moe else quant_config`. New version parses `quantization_config.ignore` and picks `quant_config=None` only when the recipe's ignore list contains `mlp.{gate,up,down}_proj` (case a, BF16-preserved dense MLP). Otherwise passes `quant_config` (case b, AWQ-quantized dense MLP — what `mattbucci/gemma-4-26B-AWQ` HF mirror has). On Ampere, the new logic closes the 30-min-after-v0.5.11-lift pad-only bug (per-expert AWQ format created BF16 placeholders that never matched .qweight keys). On R9700, the original RDNA4-sampler-HSAIL was triggered by the hardcoded `=None` branch; the detection version picks `quant_config` for our recipes (empty ignore), equivalent to no patch — should NOT re-trigger HSAIL. **Still flagged "ported, not yet applied" via setup.sh until verified post-env-rebuild (task #25).** Verification target: env rebuild to v0.5.11 stack → apply patch 023 + 024 + 028 → smoke `mattbucci/gemma-4-26B-AWQ` HF mirror at TP=1/4K → expect content-aware vision parity with 3090's 4/4 PASS. If HSAIL re-triggers on RDNA4 even with detection logic, retreat to `setup.sh` skip. |
