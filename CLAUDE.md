...
- **The live serving tree is `/data/sgl-v0514`** (SGLang **v0.5.14** + 46 RDNA4 patches, editable install in env **`sglang-triton36-v0514`**) — promoted 2026-06-26 (resweep 7/8 presets parity-or-better; cuda-graph fixed via launch.sh `--pre-warm-nccl`; DeltaNet class fixed by patch 073). **Rollback** = the retained, untouched v0.5.13.post1 stack: tree `/data/sgl-rebase` + env `sglang-triton36-v0513` (`ENV_NAME=sglang-triton36-v0513 SGLANG_DIR=/data/sgl-rebase scripts/launch.sh …`). Older v0.5.12 rollback: `/data/vG` + env `sglang-triton36`. NOT `components/sglang` (stale rebase workspace). Patch live behavior in `/data/sgl-v0514`; capture every live edit into `patches/` immediately. Equivalence gate after any patch edit: apply series to a pristine `v0.5.14` clone (`git worktree add`), `git add -A && git diff` vs `/data/sgl-v0514` `r9700-v0514` HEAD must be empty (see `patches/v0514-rebase-2026-06-26.md`).
