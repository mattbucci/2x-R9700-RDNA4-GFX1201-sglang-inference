...
- **The live serving tree is `/data/sgl-v0515`** (SGLang **v0.5.15** + 47 RDNA4 patches, editable install in env **`sglang-triton36-v0515`**) — promoted 2026-07-11 (rebased from v0.5.14: 547-commit upstream delta, only 4 conflicts, 064 upstreamed; equivalence gate byte-equal; import smoke 29/29; GPU coder-30b 35/36 + qwen36-moe caps 3/3 + gemma4 4/4-incl-video + North-Mini 062; cuda-graph pre-warm-nccl holds; patch 073 relocated to `arg_groups/overrides.py`). **Rollback** = the retained, untouched v0.5.14 stack: tree `/data/sgl-v0514` + env `sglang-triton36-v0514` (`ENV_NAME=sglang-triton36-v0514 SGLANG_DIR=/data/sgl-v0514 scripts/launch.sh …`). Older rollbacks: v0.5.13.post1 = `/data/sgl-rebase` + `sglang-triton36-v0513`; v0.5.12 = `/data/vG` + `sglang-triton36`. NOT `components/sglang` (stale rebase workspace). Patch live behavior in `/data/sgl-v0515`; capture every live edit into `patches/` immediately. Equivalence gate after any patch edit: apply series to a pristine `v0.5.15` clone (`git worktree add`), `git add -A && git diff` vs `/data/sgl-v0515` `r9700-v0515` HEAD must be empty (see `patches/v0515-rebase-2026-07-11.md`).
