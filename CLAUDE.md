...
- **The live serving tree is `/data/sgl-rebase`** (SGLang **v0.5.13.post1** + 34 core patches + 3 fixes, editable install in env **`sglang-triton36-v0513`**) — promoted 2026-06-16. **Rollback** = the retained, untouched v0.5.12 stack: tree `/data/vG` + env `sglang-triton36` (`ENV_NAME=sglang-triton36 SGLANG_DIR=/data/vG scripts/launch.sh …`). NOT `components/sglang` (stale rebase workspace). Patch live behavior in `/data/sgl-rebase`; capture every live edit into `patches/` immediately (the 2026-06-10 audit found two uncaptured edits + a stale workspace with `.rej` leftovers). Equivalence gate after any patch edit: apply series to a pristine `v0.5.13.post1` clone (`git worktree add`), `git add -A && git diff` vs `/data/sgl-rebase` HEAD must be empty (see `patches/v0513-rebase-2026-06-16.md`).
