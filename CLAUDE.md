...
Custom SGLang v0.5.15 + RDNA4 patches for 2x AMD Radeon AI PRO R9700.
...
## Key Commands
```bash
scripts/setup.sh                       # Full setup (applies the numbered patch series — index: patches/README.md)
...
- **The live serving tree is `/data/sgl-v0515`** (SGLang **v0.5.15** + **56 RDNA4 patches**, editable install in the fresh env **`sglang-triton36-v0515`**) — promoted 2026-07-11 with 47 gate-verified patches, then extended 2026-07-12 with 074–082 for North Mini Code/Laguna FP8-KV correctness, model-aware MoE tuning, fused HIP routing, Laguna BF16 gate/attention collective, HIP Triton RMSNorm/q-k norm, and fused static-scale FP8 K/V cache stores. Focused receipt: `patches/v0515-north-laguna-2026-07-12.md`; original historical rebase: `patches/v0515-rebase-2026-07-11.md`. **Rollback** = the retained, untouched v0.5.14 stack: tree `/data/sgl-v0514` + env `sglang-triton36-v0514` (`ENV_NAME=sglang-triton36-v0514 SGLANG_DIR=/data/sgl-v0514 scripts/launch.sh …`). Older rollbacks: v0.5.13.post1 = `/data/sgl-rebase` + `sglang-triton36-v0513`; v0.5.12 = `/data/vG` + `sglang-triton36`. NOT `components/sglang` (stale rebase workspace). Patch live behavior in `/data/sgl-v0515`; capture every live edit into `patches/` immediately. Equivalence gate after any patch edit: replay the numbered series onto pristine v0.5.15, compare it byte-for-byte with the intended live delta, and run the no-double-apply check.
- **Final North/Laguna focused results (2026-07-12):** North 71.053 / 60.714 / 42.298 / 33.905 tok/s at inputs 128 / 29,357 / 117,048 / 219,352, all coherent; its OFF control was hardware/allocator-unstable, so do not quote a fused-KV percentage. Laguna's conservative reverse-confirmed patch-082 medians are 48.999 / 47.485 / 39.959 ON vs 47.772 / 39.342 / 27.202 OFF (+2.57% / +20.70% / +46.89%), plus 29.270 tok/s coherent at input 220,277. Full method and quality caveats: `benchmarks/north-laguna-v0515-r9700-2026-07-12.md`.
...
- **Spec-decode `@256K` numbers measure server *capability*, not at-depth throughput — at TRUE 256K decode depth on real content, spec COLLAPSES (R9700 2026-06-15).** The README next-step #2 was "re-measure the working spec bars at full 256K" — assumed a lower-priority confirmation. It instead *disconfirmed* the bars. Method: decode at ~244K depth on diverse real SGLang source (not `measure_decode_curve.py`'s repetitive filler, which trivially inflates draft acceptance; not a short prompt on a 256K-capable server), reading server-log `gen throughput`. Decisive isolator = an **in-session depth A/B** (one server boot, two prompts, only depth varies): **Coder-30B EAGLE3 AWQ — short 107 t/s acc 6.12 → 244K-depth 0.8 t/s acc 1.75**, **~15× slower than no-spec @244K (12.3, measured clean, content-independent)**; **Qwen3.6-35B DFlash AWQ — ~1.2 t/s @240K** (vs doc'd 80). **Universal across architecture (pure-attention MoE *and* DeltaNet-MoE) and draft (EAGLE3 *and* DFlash)** → this is broader than the DeltaNet-verify-wall bullet below; the two compounding causes are (1) draft acceptance craters at depth (the draft is short-ctx-calibrated, can't predict hard real-code continuations at 244K — accept 6.12→1.75) and (2) the draft attends the full 244K KV every micro-step (6 draft fwds + verify per step), so the spec step costs ~15× a plain decode for *fewer* tokens. The documented `@256K` bars were self-evidently short-depth: the Coder-30B "97 @256K" commit (`f9bf29d`) records `max_total=817979, huge KV headroom` (a near-empty pool = shallow decode on a 256K-*capable* server). **Implications:** for the single-user 256K mandate, **no-spec is the path at depth; spec is a ≤~32–64K optimization only**; the Qwen3-VL-32B EAGLE3 draft train (~27 H20-GPU-hr) is no longer a "256K win" (its dense→no-verify-wall premise is refuted — pure-attention spec collapses too). Measure spec at depth on REAL content (`scripts/bench/spec_depth_ab.sh`), never filler or short-prompt-on-big-server. Full data + harnesses: `benchmarks/spec-at-depth-collapse-2026-06-15.md`. The harness footgun that cost an hour here: a boot-readiness grep of `sigquit` (case-insensitive) false-matches the benign `custom_sigquit_handler=None` in sglang's normal `server_args` dump → it kills the *healthy* booting server (the resulting BrokenPipe init-handoff crash is the symptom, not the cause); use `Received sigquit`.
